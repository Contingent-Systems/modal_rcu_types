...
In Figure \ref{fig:del2}, we show the steps for deleting the heap node $H_1$. To locate the node $H_1$, as shown in Figure \ref{fig:del2.1}, we first traverse the subtree $T_0$ with references $pr$ and $cr$, where $pr$ is the parent of $cr$ during traversal:
\[ pr:rcuItr(l|r)^{k} \{l|r \rightarrow cr\},\, cr:rcuItr(l|r)^{k}.(l|r) \{\}\]
Traversal of $T_0$ is summarized as $(l|k)^{k}$. The most subtle aspect of the deletion is the final step in the case the node $H_1$ to remove has both children; as shown in Figure \ref{fig:del2.2}, the code must traverse the subtree $T_4$ to locate the next element in collection order: the node $H_s$, the left-most node of $H_1$'s right child ($sc$) and its parent ($lp$):
