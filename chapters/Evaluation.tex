...
;
\end{tikzpicture}
\caption{Heap node $H_1$ and its successor $H'_1$.}
\label{fig:del2.1}
\end{subfigure}\quad
\begin{subfigure}[b]{.4\linewidth}
\centering
...
;
\end{tikzpicture}
\caption{Duplicating $H'_1$ as a fresh heap node before replacing $H_1$ with the fresh one.}
\label{fig:del2.2}
\end{subfigure}\quad
\begin{subfigure}[b]{.4\linewidth}
\centering
...
\tikzset{
  ...
  blue node/.style = {rectangle,draw=green,inner sep=1.5,dashed}
    ...
    \node[hollow node]            (kp1) [below right of = t0]   {$H'_1$};    
    \node[subtree]             (kl)    [below left of=k]      {$T_{2}$};
\node[hollow node]          (kr)    [below right of = k]  {$H_2$};
 
  \node[rsubtree]          (rlmst)    [below left of = kr]  {$T_4$};
   \node[subtree]  (t5) [below right of = rlmst] {$T_5$};
    \node[rhollow node]             (krl)         [below left of = rlmst]        {$H'_1$};
    \node[succn]          (nl)    [below left of = krl]  {};
    \node[subtree]          (krr)       [below right of = kr]           {$T_{3}$};
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\node[hollow node]          (kr)    [below right of = k]  {$H_2$};
  ...
  \node[rsubtree]          (rlmst)    [below left of = kr]  {$T_4$};
   \node[subtree]  (t5) [below right of = rlmst] {$T_5$};
    \node[rdhollow node]             (krl)         [below left of = rlmst]        {$H'_1$};
    \node[succn]          (nl)    [below left of = krl]  {};
    \node[subtree]          (krr)       [below right of = kr]           {$T_{3}$};
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    \node[ref node] (pr) [above right of= t0] {$pr$};
...
Our implementation is a slightly simplified version of the Citrus BST~\cite{Arbel:2014:CUR:2611462.2611471}: their code supports fine-grained locking for multiple writers, while ours supports only one writer by virtue of using our single-writer primitives. For lack of space the annotated code is only in Appendix \ref{appendix:bst_del}, but it motivates some of the conditional-related flexibility discussed in Section \ref{subsection:type-action}.
