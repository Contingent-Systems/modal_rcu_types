...
is vacuous, unprovable, or unstatable, so any proof case discharged against it
establishes nothing; \textsc{major} means ill-formed but with recoverable
intent; \textsc{minor} means a typo, an unused binder, or a caption that
...
\paragraph{5.\ ULKR --- Unlinked Reachability.\hfill\textsc{minor}}
\[
\textbf{ULKR} \defn \forall o,o',f',\mathit{tid} \ldotp
  \unlk\,\mathit{tid} \in O(o) \land \sigma.h(o',f') = o
  \implies \unlk\,\mathit{tid} \in O(o') \lor \frbl\,\mathit{tid} \in O(o')
\]
\emph{Changed:} tagging only.  The caption says ``all heap locations from which
you can reach the \unlk{} one'', but the formula is the one-step version, which
is the right primitive --- the transitive statement is a derived lemma
(\textbf{ULKR}$^{*}$, by induction on path length) and should be recorded as
such rather than conflated with the invariant.

\paragraph{6.\ FLR --- Free List Reachability.\hfill\textsc{major}, \textsc{open}}
\[
...
\]
\emph{Changed:} per (C3), including the $T$ capture.  \textbf{The direction of
$\bowtie$ needs your adjudication and is left open} --- see Section
\ref{sec:inv-open}.
...
\item \textbf{UNQR (\S17)} --- \textsc{Unlink}, \textsc{Replace},
  ...
  \itr{} that would conflict with \textbf{UNQR}''.  This is the bulk of the
  remaining work and it is genuine proof, not transcription.
...
\item \textbf{HD (\S15)} --- \textsc{Replace}, \textsc{ReadHeap},
  \textsc{WriteFreshField} have no proof text at all.  These need writing from
  scratch, but each should be short: allocation extends $dom(h)$, field writes
  store a value that \textbf{HD} already places in $dom(h)$, and \texttt{Free}
  needs the case where the freed location was not the target of any surviving
  edge, which is \textbf{ULKR}.

\item \textbf{WULK, IFL, FLR, RINFL, UNQRT} --- restatement only, provided the
  \textsc{open} question in \S\ref{sec:inv-open} resolves in favour of the
  reading already used in the proofs.  Recheck each citation site rather than
  assuming.
\end{enumerate}
...
\paragraph{Direction of the inclusion in FLR (\S6).}
The original concludes $\exists T' \subseteq T \ldotp F(o' \mapsto T')$: the
predecessor's reader set is a \emph{subset} of the successor's.  We cannot
derive which direction is intended and have deliberately left $\bowtie$
abstract rather than guess, because the two readings give different invariants
and one of them may be what the \textsc{Unlink} proof actually needs.

The tension: a free-list entry is populated at \texttt{SyncStart} with the
then-current reader set and shrinks as readers execute \texttt{ReadEnd}.  Along
a chain of unlinked nodes $o' \to o$, \textbf{ULKR} says $o'$ was unlinked no
later than $o$.  If ``no later'' means strictly earlier, $o'$ has had more time
to drain and $T_{r}' \subseteq T_{r}$ would be the defensible direction --- the
opposite of what is written.  If both were unlinked in the same critical section
under one grace period they are equal.  Resolving this needs the intended
temporal story, which only you have.

\paragraph{Whether AWRT should be derived rather than assumed.}
See \S\ref{sec:inv-iris}.

\subsection{Recommendation for the Iris encoding}
\label{sec:inv-iris}

Several of these invariants exist only because the model represents the
structure as a raw heap plus a reachability function $h^{*}$, and must then
\emph{assert} that the result is a tree.  If the mechanization instead carries a
representation predicate $\textsf{tree\_rep} : \Loc \to \textsf{tree} \to
\textsf{iProp}$, with paths as positions within it, then

\begin{itemize}
\item \textbf{UNQR} (\S17) --- unique reachability --- is a property of positions
  in an inductive tree, and is \emph{derivable};
\item \textbf{UNQRT-a} (\S16) --- no incoming edge to the root --- likewise;
\item \textbf{OW} (\S1) --- in-degree at most one for live nodes --- likewise;
\item \textbf{HD} (\S15) --- domain closure --- becomes structural.
\end{itemize}

That converts the four invariants most likely to be quietly violated by a proof
step into lemmas the representation gives you, and it removes the need to
re-establish them by hand in every atomic-action case.  The residue --- the
observation map $O$, the free list $F$, the grace-period invariants
(\textbf{IFL}, \textbf{RINFL}, \textbf{FLR}) and the fresh-node invariants
(\textbf{FR}, \textbf{WF}, \textbf{FNR}, \textbf{FPI}) --- is genuinely about
ghost state and stays as ghost state.

The cost is that unlinked-but-not-yet-freed nodes live \emph{outside} the tree
while readers may still hold them, so the invariant becomes
$\textsf{tree\_rep}\;rt\;t \ast \big(\ast_{o \in dom(F)} \textsf{detached}\;o\big)$
rather than a single predicate.  That split is not incidental --- it is exactly
the delayed ownership transfer, and having it visible in the invariant's
structure is an argument for this encoding rather than against it.
