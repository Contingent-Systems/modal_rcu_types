...
here rather than eighteen times below.
...
$\textsf{WellFormed}(\sigma,O,U,T,F)$ is the conjunction of the eighteen
...
All eighteen are mechanized in \texttt{rocq/WellFormed.v}, which also carries
...
\paragraph{18.\ UNQR --- Unique Reachability.\hfill\textsc{critical}}
\[
...
\paragraph{Machine-checked status.}  Eight of the defects above are mechanized
...
the other seventeen invariants and the published \textbf{FNR} in which a node
...
for \textbf{WUNLK} --- a state satisfying the other seventeen in which a node is
unlinked by a thread that is not the one holding the lock, so that the
\textbf{ULKR} case of the two unlinking rules cannot close.  A ninth,
\texttt{FPI\_not\_preserved\_by\_replace}, is a defect in
\textsc{T-Replace} and \textsc{T-UnlinkH} rather than in an invariant
statement; both rules now carry the repairing premise.  The remaining
...
Five of the nine were found by attempting a proof rather than by reading ---
...
support \textbf{ULKR} under linking, and \textbf{WUNLK}'s absence, which is the
same case one step further along --- and all five are cases the report calls
...
repaired premise is used exactly once in it, at the step that would otherwise
...
Seventeen of the eighteen fall out of two one-line facts, that freeing removes
...
the heap are untouched outright.  \textbf{HD} is the only one whose conclusion
is about the heap, and it is the only one with any content --- and it needs the
corrected statement, since under the published \textbf{HD} the theorem is false.
So the report's verdict on the case was right about seventeen eighteenths of it
...
Every one of the eighteen cases is one of those two applied once.
...
only place in the development where it is load-bearing --- which is worth
recording, given that \textbf{RITR} as published is unsatisfiable in exactly
the states where it is needed.  \textbf{WNR} likewise earns its keep here and
only here: the writer is not the departing thread, so its \itr{} observations
are among those that survive, which is what \textbf{FPI} and
\textbf{UNQRT}$_b$ need.

Finally the three heap mutations, so every atomic action of the system now
preserves \textbf{WellFormed} as a step.  They are harder than the other three,
...
\textsc{Free}, \textsc{SyncStop} and \textsc{ReadEnd} the ten heap-free
...
again in the \textbf{WULK} case of \textsc{T-Insert} and \textsc{T-Replace},
where the node just promoted to \itr{} has to be shown to carry neither \unlk{}
nor \frbl{}.  And \textbf{WUNLK} is the one thing the assembled cases needed
that the published set did not have.

One structural observation came out of doing the breadth.  Ten of the eighteen
invariants --- \textbf{RWOW}, \textbf{AWRT}, \textbf{IFL}, \textbf{WULK},
\textbf{WF}, \textbf{FNR}, \textbf{RITR}, \textbf{RINFL}, \textbf{WNR},
\textbf{WUNLK} --- do not mention the heap, so a field write transfers them
definitionally and their cases are the identity.  The eight that do --- \textbf{OW}, \textbf{ULKR},
...
exposed is not in any of the eight --- \textbf{FNR} is one of the nine that a
...
invariant that couples the heap change to the observation change --- each rule updates
both, and \textbf{UNQRT}$_b$ is where the two have to agree --- and because
...
What remains is the Hoare triples themselves, which wait on a choice of heap
assertion for the object language rather than on anything here.
