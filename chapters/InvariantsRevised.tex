...
\paragraph{Machine-checked status.}  Five of the defects above are mechanized in
...
together with its repair \texttt{ULKR\_closed\_reaches}.  A sixth,
\texttt{FPI\_not\_preserved\_by\_replace}, is a defect in \textsc{T-Replace} and
\textsc{T-UnlinkH} rather than in an invariant statement; both rules now carry
the repairing premise.  The remaining \textsc{major} and \textsc{minor} items
are restatements whose defects are syntactic (capture, scope, unused binders)
and are visible without proof.

Two of the six were found by attempting a proof rather than by reading ---
\textbf{ULKR}'s failure to chain, and \textbf{FPI}'s failure to survive
unlinking --- and both are cases the report calls trivial.
...
Atomic-action lemmas for the three heap mutations: each preserves \textbf{UNQR}
and \textbf{HD}, and unlinking preserves \textbf{FPI}.  Each takes only
invariants of the pre-state and premises of its own rule.  The \textbf{FPI} case
is the one whose failure forced the repair to \textsc{T-Replace} and
\textsc{T-UnlinkH}, and the repaired premise is used exactly once in it, at the
step that would otherwise fail --- so that defect is now closed from both ends,
by a counterexample against the published rule and a proof for the repaired one.

What remains is the rest of \textsf{WellFormed} for each action, which is
breadth rather than depth, and the Hoare triples themselves, which wait on a
choice of heap assertion for the object language rather than on anything here.
