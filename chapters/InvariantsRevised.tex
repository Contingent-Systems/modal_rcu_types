...
the \texttt{Print Assumptions} reports: all 257 say \emph{closed under the
...
Two further ones came out of the reader's read: \S\ref{sec:defect-samesnap},
free-list entries need not agree, which \textsc{SyncStart} carries as a
hypothesis; and \S\ref{sec:defect-frw}, fresh-reachability is guarded by the
stack, which is \textbf{WFresh}'s defect one invariant along.  Two more are at
the two ends of the read side:
...
What remains of the reader's read is to carry both conjuncts in the Iris
invariant rather than as hypotheses.  \textbf{SameSnap} is the mechanical pass
the other added conjuncts took; \textbf{FRW} is not, because a field write
creates an edge and so has something to prove at each of the five mutations ---
in each case that the edge's new target is not fresh, which the two linking
rules get from promoting it in the same step and the others from \textbf{FNR}.
...
Both conjuncts are carried in the invariant now.  \textbf{SameSnap} was the
mechanical pass; \textbf{FRW} was not, and the five mutations are where the work
is, a field write being the only thing that creates an edge.

What the reader's read still wants is not an invariant but a decision.  The rule
gives the reader an observation at a node it may never have observed, and to
grant one it must hold that node's entry or know the entry absent.  Its cell
records which nodes it observes, so the first is available for those; the second
is not, because \textsc{ReadEnd} leaves the \rt{} observation behind --- that
observation is anonymous, deliberately, and so can sit in a departed reader's
entry.  Deciding that \rt{} belongs to the lock holder would let
\textsc{ReadEnd} clear its keys outright and close the read; it is the same
question as \S\ref{sec:defect-obs}'s, one observation further, and we raise it
...
writer hold a live reference to a node the writer is about to reclaim.  The step
relation is a parameter there, and deliberately: its obligation is exactly the
fifteen preservation theorems, so any relation assembled from them discharges it
by case analysis, and assembling it is transcription rather than proof.
\texttt{free\_runs\_are\_safe} instantiates it at \textsc{Free} so the
parameterisation is not vacuous.  Writing the fifteen out as one inductive
relation, and with it a progress theorem, is what is left of this part; it adds
nothing to the argument and a good deal to the file.
...
One assumption about clients that no type system discharges, and one decision
about who owns the root observation.  The
