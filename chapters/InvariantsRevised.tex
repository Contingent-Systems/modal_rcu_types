...
\paragraph{What is left, and it is four things.}
...
\emph{A decision about the root observation.}  \rt{} is deliberately anonymous,
and that is what stops \textsc{ReadEnd} from clearing its own keys, which is
what stops the reader's read from closing like the other nine rules.  Deriving
it from the structure rather than storing it in a thread's entry would settle
both at once and would be more faithful to what the published text says it is;
we have not made that change, because it is a change to the model and the
model is the paper's.

\emph{Axiom soundness proper.}  Every action preserves \textsf{WellFormed}, and
that is fifteen theorems and now an induction over runs.  Axiom soundness asks
for more: that the post-state satisfies the denotation of the post-type
environment.  Six of the fifteen have it.  This is the real remaining debt and
we would rather name it than let the invariant work stand in for it.

\emph{Transcription.}  Writing the fifteen actions out as one inductive step
relation, and proving progress alongside.  It adds nothing to the argument, as
\S\ref{sec:inv-mech} says, and a good deal to the file.
