...
\paragraph{$\mathcal{N}_1 = \mathcal{N}_2$ in \textsc{T-Replace} is weaker than the proof needs.}
...
which suggests the intended reading is that $\mathcal{N}$ is total on \RCU{} fields at
...
need strengthening so that $\mathcal{N}$ determines the object's full \RCU{} footprint.
...
\textsc{T-Insert} has the \emph{same} gap, in the same place: its
...
fresh node could point anywhere and give the inserted node a second child.  The
mechanization again takes the semantic property (\texttt{PointsOnlyAt}).
Whatever fix is chosen should cover both rules --- they are one problem, not two.
