...
the \texttt{Print Assumptions} reports: all 276 say \emph{closed under the
...
environment.  Eleven of the fifteen have it --- the whole write side, the
binding rules, \textsc{T-Free}, the reader's read, and the two block
boundaries.  Ten of those eleven are Hoare triples; the eleventh is the read,
which is the step and the environment with the edge as a hypothesis, for the
reason above.  \textsc{ReadBegin} and \textsc{ReadEnd} also compose
(\texttt{read\_section\_typed}): a section's postcondition is its own
precondition, so re-entry is a fact about resources rather than an argument.
The four that remain are \textsc{SyncStart}, \textsc{SyncStop},
\textsc{WriteBegin} and \textsc{WriteEnd}.  This is the real remaining debt and
