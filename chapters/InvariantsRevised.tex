...
\paragraph{Machine-checked status.}  Seven of the defects above are mechanized
...
the other sixteen invariants and the published \textbf{FNR} in which a node is
both \frsh{} and \frbl{} --- with \texttt{link\_freeable\_breaks\_ULKR} in
...
the case.  An eighth, \texttt{FPI\_not\_preserved\_by\_replace}, is a defect in
...
Four of the eight were found by attempting a proof rather than by reading ---
\textbf{ULKR}'s failure to chain, \textbf{FPI}'s failure to survive unlinking,
\textbf{HD}'s failure to survive \texttt{Free}, and \textbf{FNR}'s failure to
support \textbf{ULKR} under linking --- and all four are cases the report calls
...
Sixteen of the seventeen fall out of two one-line facts, that freeing removes
...
So the report's verdict on the case was right about sixteen seventeenths of it
...
Every one of the seventeen cases is one of those two applied once.
...
One structural observation came out of doing the breadth.  Nine of the seventeen
invariants --- \textbf{RWOW}, \textbf{AWRT}, \textbf{IFL}, \textbf{WULK},
\textbf{WF}, \textbf{FNR}, \textbf{RITR}, \textbf{RINFL}, \textbf{WNR} --- do
not mention the heap, so a field write transfers them definitionally and their
cases are the identity.  The eight that do --- \textbf{OW}, \textbf{ULKR},
