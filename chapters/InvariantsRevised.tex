...
\item \textbf{FPI (\S11)} --- \emph{does not hold; the rules need
  strengthening.}  Expected to be among the easiest items, this is instead the
  sixth defect.  \textsc{T-Replace} and \textsc{T-UnlinkH} both make a node
  ...
  post-environment.  Repair: extend the aliasing premise of both rules to fresh
  variables.  \textsc{Insert} is unaffected --- it unlinks nothing.  Machine
  checked as \texttt{FPI\_not\_preserved\_by\_replace}.
