...
\paragraph{$\N_1 = \N_2$ in \textsc{T-Replace} is weaker than the proof needs.}
...
supplies $\N_1 = \N_2$, equality of the two \emph{field maps}.  That is not the
same thing: a field map records only fields that have been read or set, and
neither $\llbracket \textsf{rcuItr}\,\rho\,\N \rrbracket$ nor $\llbracket
\textsf{rcuFresh}\,\N \rrbracket$ constrains a field outside $dom(\N)$ --- so
the two subtrees may differ on an untracked field while $\N_1 = \N_2$ holds.

Section \ref{sec:tslbl} says ``we presume all objects contain all fields'',
which suggests the intended reading is that $\N$ is total on \RCU{} fields at
the point of the rule, but nothing in the system enforces it.  Either
\textsc{T-Replace} needs that as an explicit side condition, or the denotations
need strengthening so that $\N$ determines the object's full \RCU{} footprint.
The mechanization takes the semantic property as a hypothesis
(\texttt{Mirrors}) and proves the case from it, so the choice is yours and
...
factored suffix is rewritten --- shortened by one segment for unlinking,
length-preserving for replacement, lengthened by one for insertion.  Isolating
that common statement is the next step, in place of three separate path
arguments.
