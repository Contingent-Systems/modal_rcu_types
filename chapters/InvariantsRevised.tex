...
statement; both rules now carry the repairing premise.  An eleventh is
\S\ref{sec:defect-fld} below; a twelfth --- that allocation makes the heap
infinite, so that no points-to assertion exists over it --- is
\S\ref{sec:inv-mech}, where attempting the last step found it; and a
thirteenth and fourteenth are \S\ref{sec:defect-obs}, found the same way one
step further on.  The remaining
...
\paragraph{Defect 11: the free list's domain is unconstrained.}
...
\paragraph{Defects 13 and 14: two observations that constrain nobody.}
...
Ten of the fourteen were found by attempting a proof rather than by reading ---
...
\textsc{SyncStart}'s other one, the infinite heap, which is what choosing a
physical-state predicate ran into, and the two unconstrained observations, which
are what the last linking rule ran into --- and seven of the ten are cases the
report calls trivial.  That is now a pattern rather than a coincidence, and it is the
...
\paragraph{Framing, and an eleventh defect.}  The Views framework requires the
...
\paragraph{Choosing \texttt{phys}, and the twelfth defect.}
...
finite.  \emph{Path} premises come from \texttt{ppt}, the fold of points-to
along a named path; what that derivation does \emph{not} use is the point ---
...
incident, and with them every heap mutation and the allocation are stated with a
type environment on both sides.  The pattern is uniform: the untouched variables
are framed by the two lemmas, and the touched ones are supplied, each being one
of three forms.  \textsc{T-WriteFH} is the smallest, changing no observation at
...
two places in the system that need one.  \textsc{ReadBegin}, \textsc{ReadEnd},
\textsc{SyncStart} and \textsc{WriteEnd} need reader-set and bounding-set
tokens, which is the lock's construction done twice more.

One last thing the field-list repair exposed, which we record rather than fix.
The mutation rules are stated with ``every field is an RCU field'', which with a
finite field list contradicts the class declaration the allocation rule needs,
so the two cannot appear in one lemma.  The honest repair is to restrict
No-Sharing's field conditions to declared fields.
