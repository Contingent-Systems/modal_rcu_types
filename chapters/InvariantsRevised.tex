...
the \texttt{Print Assumptions} reports: all 235 say \emph{closed under the
...
A further one is in \S\ref{sec:defect-samesnap}: free-list entries need not
agree, which the reader's read needs and which \textsc{SyncStart} carries as a
hypothesis.  Two more are at the two ends of the read side:
...
That is nineteen: fifteen invariants, two rule defects, two model defects.  The
...
Fifteen of the nineteen were found by attempting a proof rather than by reading ---
...
entries' having to agree, which is what the reader's read ran into --- and seven
of the fifteen are cases the report calls trivial.  That is now a pattern rather than a coincidence, and it is the
...
\texttt{read\_bound}, four lines from \textbf{FLR} and \textbf{IFL}.  What
remains of the reader's read is to carry that conjunct in the Iris invariant
rather than as a hypothesis, which is the same mechanical pass the other added
conjuncts took.
...
So what remains is one mechanical pass, carrying the agreement conjunct into
the invariant so the reader's read closes like the other rules, and one
assumption about clients that no type system discharges.  The
