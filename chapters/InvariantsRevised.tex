...
the \texttt{Print Assumptions} reports: all 293 say \emph{closed under the
...
What this does \emph{not} cover is the mutation half.  \texttt{xstep} has the
five protocol actions and not the ten heap ones, because the heap actions do not
involve the implementation at all: they are the writer's, they touch the heap
and the observation map and never the free list or the reader set, and the step
relation already has them.  Interleaving the two is taking their union; we have
