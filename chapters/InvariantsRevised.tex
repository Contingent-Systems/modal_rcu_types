...
the \texttt{Print Assumptions} reports: all 351 say \emph{closed under the
...
And the free list is constrained as a whole rather than at the thread's own
nodes, because \undf{} and \frbl{} quantify over entries the thread does not
name.  With it, \texttt{pool\_run\_ok}: from a typed pool, every reachable
configuration is a typed pool, with every thread still heading for the final
environment its type gave it.  The per-action obligation this leaves has the
same standing as ``every action is a step of the relation'' --- it is
discharged where the actions are, by the framing lemmas of
\S\ref{sec:inv-mech}, which are exactly its components.
