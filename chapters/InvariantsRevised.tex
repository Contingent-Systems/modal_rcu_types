...
the \texttt{Print Assumptions} reports: all 305 say \emph{closed under the
...
What this is not is a weak-memory account.  There is no such semantics here, no
release and no acquire, and no theorem relating a run under one to a run under
sequential consistency; reading a view as a sub-heap is an assumption about what
such a model would supply rather than a consequence of one.  It is the right
assumption for release-acquire and the wrong one for a model admitting
out-of-thin-air reads.  We report the partition because it is checkable and
because it says where that work would go.
