...
the \texttt{Print Assumptions} reports: all 316 say \emph{closed under the
...
Three things are still not proved, and we would rather name them than let the
model stand in for them.  The bridge is restricted to write-once cells, which
is the right fragment for publication and the wrong one for links --- and what
protects a reader holding a stale link is not the memory model but the grace
period, proved elsewhere and sequentially; joining the two needs the invariants
carried along a \emph{pair} of runs, and that simulation is not built.  The
semantics is over the heap only, so the registrations, which need
synchronisation for the opposite reason, are stated and not carried.  And it is
release-acquire and not weaker: every read acquires and every view only grows,
so nothing here would survive a relaxed model unchanged.
