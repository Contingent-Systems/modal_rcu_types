...
the \texttt{Print Assumptions} reports: all 229 say \emph{closed under the
...
grace period waits for.  That is a real obligation, it is not discharged by
owning anything, and it is what remains of the reader's read.  Under the epoch
reading it has a sharp form --- the reader registered no later than the node was
stamped --- and the argument for it is the one \textsc{RCU} rests on: a node is
detached before its grace period begins, detached nodes are unreachable, and a
reader that registers afterwards starts at the root.  Assembling that from
\textbf{ULKR} and \textbf{UNQRT-b} is the work; it is not a camera change.
...
So the remainder is two things, and neither is a camera.  \textbf{IFL}'s
obligation for the reader's read, and termination for the two loops.  The
