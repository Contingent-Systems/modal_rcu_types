...
the \texttt{Print Assumptions} reports: all 223 say \emph{closed under the
...
$R$, leaves $B$, and leaves every snapshot
(\texttt{e\_end\_simulates}).  Entering joins nothing
(\texttt{e\_begin\_joins\_nothing}): a reader registers at the current epoch and
every running grace period was stamped earlier, so it is in no snapshot, which
is the property the published model has to arrange by removing threads as they
leave.  And the bookkeeping still satisfies the invariants the snapshot model is
stated over --- \textbf{RINFL} is \texttt{e\_RINFL}, and \textbf{BR}, which the
published set is missing entirely (\S\ref{sec:defect-br}), is immediate --- so
this is a refinement rather than a different system.
