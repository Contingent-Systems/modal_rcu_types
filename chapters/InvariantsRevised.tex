...
(\texttt{WellFormed\_satisfiable}), and the positive results the proof cases now
rest on are machine checked in \texttt{rocq/HeapPaths.v}: tree-shape
preservation for all three heap mutations (\texttt{UNQR\_replace},
\texttt{UNQR\_insert}, \texttt{UNQR\_unlink}), heap-domain closure for every
action that touches the heap (\texttt{HD\_h\_upd}, \texttt{HD\_h\_alloc},
\texttt{HD\_h\_free}), and the two chaining results
(\texttt{ULKR\_closed\_reaches}, \texttt{FLR\_chains}) that together keep a node
awaiting reclamation unreachable from the root.  The whole development is
axiom-free.
