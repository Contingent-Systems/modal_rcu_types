...
\textbf{Reaches} hypothesis gets \emph{weaker}, and the nine that do not mention
the heap are untouched outright.  \textbf{HD} is the only one that asserts
...
\textsc{T-Replace} does both at once, so \textbf{RWOW}, \textbf{AWRT},
\textbf{IFL}, \textbf{WULK}, \textbf{WF}, \textbf{FNR}, \textbf{RITR} and
\textbf{RINFL} all acquire content.
...
So the repair is the one the development already recorded: a snapshot's value
should be a disjoint union of tickets, one per bounding thread, so that a reader
owns its own membership and spends it.  That makes \textsc{ReadEnd}'s write
thread-local, and re-entry cannot restore a spent ticket.  The alternative that
also works is to index snapshots by read-side critical section rather than by
thread --- which is what an implementation's grace-period counter does, and
under which the intersected reading \emph{is} sound, a section identity never
being reused.  It costs a change to the state model, which the ticket does not.
...
So the remainder is a fractional heap for the read side, and a compare-and-set
and a wait loop for the two points where the protocol tests shared state.  The
