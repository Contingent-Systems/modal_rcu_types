...
That is fifteen: twelve invariants, one rule, two model defects.  The remaining
...
Eleven of the fifteen were found by attempting a proof rather than by reading ---
...
observations, which are what the last linking rule ran into --- and seven of the
eleven are cases the report calls trivial.  That is now a pattern rather than a coincidence, and it is the
...
typed} until No-Sharing's field conditions are restricted to declared fields.
That converts a deferred tidy-up into the next thing that has to be done, and it
is the clearest argument we have for doing it.
