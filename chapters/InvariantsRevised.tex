...
the \texttt{Print Assumptions} reports: all 241 say \emph{closed under the
...
\emph{Unconditional execution.}  The four non-blocking primitives are total
functions on the machine state, so each is enabled everywhere; the two blocking
ones block rather than fail, which is what the requirement asks.  There is
nothing to prove beyond their being functions.
