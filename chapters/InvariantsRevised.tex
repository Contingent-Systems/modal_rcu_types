...
the \texttt{Print Assumptions} reports: all 273 say \emph{closed under the
...
environment.  Nine of the fifteen have it --- the whole write side, the binding
rules, \textsc{T-Free} and the reader's read.  Eight of those nine are Hoare
triples; the ninth is the read, which is the step and the environment with the
edge as a hypothesis, for the reason above.  The six that remain are the
protocol actions: \textsc{ReadBegin}, \textsc{ReadEnd}, \textsc{SyncStart},
\textsc{SyncStop}, \textsc{WriteBegin} and \textsc{WriteEnd}.  This is the real
remaining debt and we would rather name it than let the invariant work stand in
for it.
