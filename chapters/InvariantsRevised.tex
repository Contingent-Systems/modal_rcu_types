...
the \texttt{Print Assumptions} reports: all 307 say \emph{closed under the
...
What is still not here is the account itself: an operational model in which
view, release and acquire are \emph{defined} rather than assumed, and a theorem
that a run under it is matched by a run of \S\ref{sec:inv-mech}'s relation.
Everything above is about the \emph{statement} of that theorem --- which
invariants it must re-establish (one), where it must synchronise (three places),
and what it must assume about views (that they are sub-heaps and sub-maps).  We
would rather publish the obligations than an account that quietly assumed
them.
