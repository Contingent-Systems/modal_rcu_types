...
the \texttt{Print Assumptions} reports: all 395 say \emph{closed under the
...
the node read.  What remains a hypothesis there is the edge itself, because
that rule is stated over the invariant's \emph{contents}; stated over the
invariant it is a triple, and \texttt{read\_typed\_closed} is it.
...
not observational, and it is not the cell read either.  An earlier draft said
it was --- that a reader must \emph{learn} what the field holds and the
points-to assertion is exclusive --- and that too was wrong, for the reason the
correction above already gives: the authority is in the invariant, the read is
one access inside it, and nothing is carried out.  What was actually in the way
is the \emph{bind}, and \texttt{rocq/Closed.v} closes it; see
``What is left, and it is one thing'' below.
...
\paragraph{What the kernel memory model's RCU axiom is, at this interface.}
Two projects hide under ``verify against the LKMM'', and they have quite
different answers.  One is to implement RCU from counters and verify that
implementation under the model; that is the larger project, and it is worth
noting that Tassarotti et al.\ chose release-acquire over the LKMM precisely to
keep it tractable.  The other is to take the model's RCU guarantees as given
and ask what they buy a client of the interface above.  This is the second.

The LKMM does not \emph{derive} RCU from anything: it axiomatises it, and the
axiom is that a read-side critical section does not span a grace period ---
Alglave et al.'s requirement~1, and already a theorem of the epoch model.  So
the question is not whether the axiom proves something but which part of the
obligation it is, and the answer is a split, stated as an iff so that it says
both halves.  Of the thirteen clauses of \texttt{Refines}, ten are
bookkeeping: they say the four actions move the three observable components the
way the published model's do, and no implementation of RCU can get them wrong
and still be an implementation of RCU.  The other three are the axiom read
three ways --- a grace period waits for the sections in progress when it
begins, it is over only when every one of them has ended, and therefore a node
detached before it began may be reclaimed after it ends.
\texttt{refines\_split} is that iff, \texttt{lkmm\_conformance\_is\_refinement}
the direction a reviewer asks for, and
\texttt{epochs\_satisfy\_the\_axiom} the check that the statement is not
vacuous.

That leaves a translation, and it is a real one rather than a formality.  The
LKMM is a predicate on whole \emph{executions} and \texttt{Impl} is a state
machine.  An execution, at this interface, is the sequence of RCU events it
contains --- section entries and exits, grace-period starts and stops,
reclamations --- and conformance enters as a guard on each: an execution that
reclaims a node with a section still standing is one whose reclamation event is
not \emph{enabled}, and the axiom is what rules those out.
\texttt{execution\_replays}: an execution all of whose events are enabled
replays as a run of the paired step relation.  \texttt{lkmm\_execution\_is\_a\_run}:
so any execution of a conforming implementation keeps the published model in
step, which is the claim ``an implementation whose guard means what the axiom
says satisfies the interface'' was short of.  And
\texttt{executions\_free\_only\_quiesced\_nodes} says it at every
reclamation in the execution rather than only at its end.

Two things are explicit rather than assumed away.  Reading an execution as a
\emph{sequence} assumes a total order, which is not free in a weak memory
model; what makes it available is that all five events touch the protocol's own
state, so the model's coherence on that state orders them.  And the hypothesis
is a conjunction of guards, which could be unsatisfiable, so
\texttt{around\_enabled} exhibits a whole round at the counter implementation
--- a reader enters and leaves, a writer detaches a node, waits, reclaims it ---
end to end.

The same translation runs once more at the client's level, which is where the
safety theorem lives.  The protocol-level reading is the right object for an
axiom about grace periods, and it is not yet the statement a client wants,
because a client's execution contains its own loads and stores.  So an event
there is one of the five protocol events or one heap mutation, carrying what the
corresponding \texttt{xstep} constructor leaves existential --- not a
weakening, since an execution \emph{is} a record of what happened, so an event
that names its own outcome is the faithful reading and enabledness is then
exactly the constructor's premises.  \texttt{xexecution\_replays} and
\texttt{conforming\_executions\_are\_memory\_safe}: any execution of any
client against an implementation conforming to the axiom, all of whose events
are enabled, reaches only states in which no thread but the writer holds a live
reference to a node the writer may reclaim.  Conformance enters in one place ---
the reclamation event's guard --- and nothing else about the implementation is
used.  \texttt{epoch\_executions\_are\_memory\_safe} is that chain from the
axiom to the client in one statement, and \texttt{xround\_enabled} is the
non-vacuity check again, a whole client round with the writer's unlinked
observation recoloured by \textsc{SyncStop}, so that the reclamation guard is
...
One thing about that account was stronger than Linux, and we have since
repaired it rather than left it as a caveat.  Every read in it acquires, and
there was no relaxed load at all.  Linux is not that.
\texttt{rcu\_assign\_pointer} \emph{is} a release store, so the publishing half
matches; but \texttt{rcu\_dereference} is \texttt{READ\_ONCE} plus a compiler
barrier, and what orders the access is the address \emph{dependency} between
loading the pointer and loading through it.  An acquire orders the load against
everything the publisher knew; a dependency orders it only against accesses
that use the value --- the target's own cells, and nothing else.

So there is a second relation, with the same releasing write, a relaxed read,
and a dependency-ordered read that acquires at the target only.  Both are kept,
because having both is what lets the difference be stated, and the difference
is sharp.

Under release-acquire every reader's view is one of the writer's past views.
Under dependency ordering that is false, and we give the run: the writer points
the root's second field at 5, then at 6, then the first field at 5, so
unique-paths holds at every moment; a reader that dereferences the second field
while it still reads 5 and then dereferences the first is carried forward only
at node 5's cells, not at the root's, so it keeps the stale field.  Its view has
both fields at 5, which no state ever had, and in that view 5 has two paths.
That is the exact price of the weaker read.

What survives is what is needed, and the coincidence is worth naming.  The
guarantee under dependency ordering is not a property of a configuration --- a
thread may come to hold a link it never dereferenced, and nothing orders the
target of a pointer that was never followed --- but of the \emph{step}: after
an \texttt{rcu\_dereference} of $n$, the reader sees whatever the publisher of
that pointer had already written into $n$.  Heap-domain closure, the one
invariant a weak model endangers, says a thread that can see an edge can see
the node at the end of it, and a thread that has not followed the edge has no
business seeing the node.  The per-dereference statement is the obligation with
nothing left over.

What still stands is that this is release-acquire and dependency ordering, and
not a relaxed model: every view only grows, so there is nothing out of thin air
to rule out.  And source-level dependencies are not guaranteed by C --- a
compiler may break them by value speculation or by arithmetic that cancels ---
so the kernel memory model formalises the kernel's assumptions about its
toolchain rather than standard C semantics.  A proof against it is a proof
against a model in which the compiler is trusted, not proven, to respect
dependencies.  We say that rather than absorb it.
...
\paragraph{Programs, at last.}
Everything above relates \emph{states}.  \S\ref{sec:inv-mech}'s relation is
over \textsf{LState}, the fifteen action theorems are about one state and the
next, and the safety theorem says ``any state reachable by these steps''.  That
is weaker than it sounds: there is no command language anywhere in the
development, no thread pool, and no configurations, so there is no way to
quantify over the runs of a \emph{program}.

\texttt{rocq/Programs.v} supplies them: a command language whose primitives are
the fifteen actions, with the two block forms derived rather than primitive, as
the desugaring says they are; a thread pool; configurations; a thread-local step
relation and its lift to an interleaving.  The theorem is the one the earlier
files could not state --- at no point in any interleaving of any program, from
any well-formed start, does a thread other than the writer hold a live
reference to a node the writer is about to reclaim.  Typing is structural, with
the action case a parameter whose hypothesis is exactly axiom soundness, so the
file does not re-prove the type system but says what its per-action results add
up to once there are programs; subject reduction holds.

Two abstractions are stated rather than hidden.  Conditions are abstracted, a
conditional stepping to either branch: there is no expression language here,
and for safety taking either branch quantifies over \emph{more} runs than a
real condition would allow.  And the action's effect is a parameter required
only to be a step of \S\ref{sec:inv-mech}'s relation, which is what lets the
file be about programs without re-encoding fifteen actions' side conditions.
Pool-level preservation of typing needs one thing more.  One thread's step
changes shared state, so something must say the other threads' environments
survive it, and that condition is read off the denotations rather than guessed:
a type's meaning for a thread mentions that thread's own stack slots, the
observations tagged with it, the root observation, its scope, the free list,
the heap, the lock and the root, and nothing else.  \texttt{Frames} is those
eight components and \texttt{Frames\_D\_env} is the proof that the list is
complete, one case per type.  It is deliberately not minimal per type --- an
\undf{} reference needs far less --- because a frame condition has to cover an
\emph{environment}, which may hold any of the six.

What it constrains says two things.  The heap, the lock and the root are
constrained as wholes, which makes the writer's own actions non-framing; that
is correct rather than a weakness, since an unlink really can invalidate
another writer's path, and preventing exactly that is what the lock is for.
And the free list is constrained twice and weakly --- entries that are absent
stay absent, entries with nobody left to wait for keep having nobody --- rather
than by an equality, because those are the only two things the denotations ask
of it and an equality would exclude \textsc{ReadEnd}, which does nothing to the
free list but shrink its entries.  With it, \texttt{pool\_run\_ok}: from a
typed pool, every reachable configuration is a typed pool, with every thread
still heading for the final environment its type gave it.

The per-action obligation is then discharged rather than left a hypothesis, and
the answer is not uniform, which is the part worth reporting.
\texttt{Frames\_machine}: a step that changes only the reader or bounding set
frames \emph{every} thread, because neither set is mentioned by any denotation
--- \textsc{ReadBegin}, and the machine halves of \textsc{SyncStart} and
\textsc{SyncStop}.  \texttt{frames\_read}: a reader's read extends its own
column of the observation map, so it frames every other thread, and the
well-formedness condition on that map is exactly what separates the columns.
\texttt{frames\_bind}: \textsc{Bind} writes one stack slot.
\texttt{frames\_read\_end} frames too, and needs one hypothesis the abstract
step does not supply --- that the other threads' scopes are unchanged.  That is
an under-specification of the action rather than a gap in the frame condition,
and it is better as a hypothesis in a theorem than as a remark.

The writer's ten do not frame, and \texttt{heap\_changes\_do\_not\_frame}
is the witness rather than the assertion.  That is correct, and
\texttt{heap\_types\_need\_the\_lock} and
\texttt{heap\_types\_are\_exclusive} say why: every type whose denotation
constrains the heap requires the lock, so no two threads hold one at the same
time, and a writer's mutation therefore has no \emph{other} thread's path type
to invalidate --- the thread that could hold one is the writer itself.  The
protocol's answer to framing is the lock, and the frame condition is where that
becomes visible instead of assumed.

Two smaller things in the same file.  Safety and progress are separate and both
are now stated: \texttt{tstep\_progress}, that a non-skip command steps unless
the action it is about to perform cannot.  That is the right statement rather
than a weak one, because an action whose guard is false genuinely blocks and
should --- a writer waiting on the lock, a grace period waiting on a reader ---
and with \texttt{finished\_pools\_are\_stuck} a stuck configuration is
finished or blocked on a guard and never a third thing.  And the action's
effect, a parameter throughout, is instantiated with
\S\ref{sec:inv-mech}'s relation itself, so
\texttt{real\_programs\_are\_memory\_safe} is about the fifteen actions
rather than about an abstraction of them.

\paragraph{What is left, and it is one thing.}
It used to be three.  ``Axiom soundness proper'' went first: all fifteen actions
have it.  The other two are done in \texttt{rocq/Closed.v}, and both turned out
smaller than they had been stated --- in one case because the diagnosis was
wrong.

\emph{Fairness}, which is what is left.  That a reader inside a critical
section eventually leaves it.  The wait's termination reduces to it and no
further --- it is a property of the client, and no invariant of the shared state
could supply it.

\paragraph{The reader's read, and a diagnosis corrected.}
It was said here that the read was not a Hoare triple because a reader must
learn what a field holds and the points-to assertion is exclusive, and that the
repair was a fractional or discardable-fraction heap.  That was the wrong
obstacle.  The authoritative heap is \emph{in} the invariant, so a reader that
opens the invariant reads the edge there; and a reader's \itr{} mentions no
cell, so nothing has to be carried out past the closing.  A fraction buys
something only if it does.

What was actually in the way is one step further on, and it is specific to RCU.
The rule moves the reader's stack, so the machine's stack has to move with it,
and the general binding lemma asks the bound node to be neither detached nor
awaiting reclamation.  A reader can promise neither: holding a reference to a
node a writer has already unlinked is not an edge case of RCU but the whole of
it.  So the reader's bind is a different lemma,
\texttt{reader\_bind\_preserves\_WellFormed}, and it is a smaller one for a
reason worth recording.  With the observation already granted by the read, the
bind changes only the stack and the scope, and exactly four of the twenty
conjuncts mention those --- \textbf{RWOW}, \textbf{AWRT}, \textbf{FR} and
\textbf{WFresh}.  The other sixteen are identities, because the observation
map, the free list, the heap, the lock and the root are untouched.  A reader's
bind cannot break an invariant about detachment or reclamation, which is why it
needs no premise about either: the general bind's two premises are not weakened
here but \emph{absent}, because they were never the bind's business.  They
belong to the writer's bind, whose post-type is an \itr{} with a path and a
field map and whose denotation asks for both.

\texttt{read\_atomic} is then the rule against the invariant --- the edge read
rather than assumed, the machine moved by the bind, the invariant
re-established --- and \texttt{read\_typed\_closed} is it with the reader's
environment on both sides, where the iterator at $x$ comes out of the
environment rather than in as a hypothesis about the observation map.

\paragraph{The enumeration premises, discharged by finiteness.}
Four rules were stated as a step plus an environment with an enumeration made
explicit: the bulk updates, where ``every detached node is stamped'', ``every
reachable node is observed'', ``every entry of the writer's is recoloured'',
``every entry is given up''.  It was said that no resource a thread holds
discharges those.  That conflated two things, and only one of them is true.

Whether the thread \emph{owns} the pieces is a resource question and stays one;
that is what ownership is, and a bulk update that did not have to own what it
updates would be unsound.  But whether the set is \emph{enumerable} is a
mathematical question, and the answer is yes in all four cases, for the same
reason: every one of these sets is a set of keys of a finite map the invariant
already carries.  The heap is a \texttt{gmap}, the observation map is a
\texttt{gmap}, and a set carved out of one by a decidable condition is finite.

Two of the three lists are then immediate.  \texttt{detached\_list} is the
nodes with a detaching observation, read off the observation map, and
\texttt{column} is the writer's column of it --- so \textsc{SyncStop}'s and
\textsc{WriteEnd}'s two clauses relating the thread's map to the shared one
hold by \texttt{column\_lookup}, definitionally.

The third is not immediate.  \textsc{WriteBegin}'s set is the \emph{reachable}
nodes, and reachability is a closure rather than a lookup, so it has to be
computed and the computation shown to terminate at the right answer.  It does:
iterate ``add the successors'' from the root, and iterating it as many times as
there are locations the heap mentions at all is enough.  That last step is a
pigeonhole --- the iterates increase, they all live in a finite set, so one of
the first $n+1$ is a fixed point and nothing changes after one --- and it is
stated on its own as \texttt{chain\_stabilises}, because it has nothing to do
with heaps.  With \texttt{reach\_set} in hand, \texttt{wb\_list} is the
writer's entry at every reachable node and the rule's six enumeration clauses
are readings of its soundness and completeness.

So the four rules now ask for the conditions the action itself carries, for the
resources, and for nothing else.  \texttt{sync\_start\_enumerated},
\texttt{sync\_stop\_enumerated}, \texttt{write\_begin\_enumerated} and
\texttt{write\_end\_enumerated} are the four, at their own lists.
