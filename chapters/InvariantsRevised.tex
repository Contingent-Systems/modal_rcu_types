...
the \texttt{Print Assumptions} reports: all 287 say \emph{closed under the
...
What this does not yet do is transfer memory safety.  The paired step relates an
implementation to the published model's machine state and free list; it does not
yet touch the observation map, so it does not yet compose with the step relation
of \S\ref{sec:inv-mech} and the safety theorem stated there.  That composition
is the remaining research, and we would rather say so than let the interface
stand in for it.

\paragraph{What is left, and it is three things.}
The third of them has changed shape since the last draft, and is now a list of
premises rather than a list of rules.
...
environment.  All fifteen now have it.  Ten of the fifteen are Hoare triples
