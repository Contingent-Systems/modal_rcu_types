...
A further invariant defect is in \S\ref{sec:defect-br}: the bounding set is
unconstrained, so nothing makes the grace period terminate.

That is seventeen: thirteen invariants, two rule defects, two model defects.  The
...
Thirteen of the seventeen were found by attempting a proof rather than by reading ---
...
program ran into, and the unconstrained bounding set, which is what giving the
read side a resource reading ran into --- and seven of the thirteen are cases
the report calls trivial.  That is now a pattern rather than a coincidence, and it is the
...
found \textbf{BR} (\S\ref{sec:defect-br}).  The reader set is a disjoint-union
camera: entering a critical section mints a thread's token, leaving it returns
one, and the token is what \textsc{ReadEnd} consumes --- so a thread cannot take
another thread out of $R$, which is what the published model's unowned $R$ left
open.  $R$ becomes finite in the process, for the same reason the heap and the
...
move $R$ or $B$, which before the token was true of no step at all.
