...
\paragraph{Machine-checked status.}  Six of the defects above are mechanized in
...
together with its repair \texttt{ULKR\_closed\_reaches}; and
\texttt{HD\_not\_preserved\_by\_free}.  A seventh,
...
Three of the seven were found by attempting a proof rather than by reading ---
\textbf{ULKR}'s failure to chain, \textbf{FPI}'s failure to survive unlinking,
and \textbf{HD}'s failure to survive \texttt{Free} --- and all three are cases
the report calls trivial.  That is now a pattern rather than a coincidence, and
it is the strongest thing the mechanization has to say about the original
argument.
...
preserves \textbf{UNQR}, \textbf{HD} and \textbf{OW}, and unlinking and
replacement preserve \textbf{FPI}.  The \textbf{FPI} case is the one whose
failure forced the repair to \textsc{T-Replace} and \textsc{T-UnlinkH}, and the
