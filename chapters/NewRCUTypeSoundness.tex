...
In this section we outline our proof type soundness.  Our full proof appears in Appendices \ref{sec:prooflemmas}, \ref{sec:memaxioms}, \ref{lem:lematom} and \ref{lem:lemstructural}.
...
\label{sec:lemmas}
In this section we summarize the important aspects of \textsf{WellFormed}ness conditions of RCU model which we introduce formally as a conjuction of \textit{global invariants} in Appendix \ref{sec:memaxioms} and the type denotations. We aim to convey an intiution that our system captures the requirements of RCU setting~\cite{abssem}.

\mypar{Grace-Period Guarantee} A mutator thread has to synchronize with all of the reader threads simultaneously running the read-side critical section to \textit{reclaim} the heap locations. The operational semantics enforce a \textit{protocol} on the mutator thread's actions :1-first it updates a heap location(e.g. \textit{unlinking}) 2-then it \textit{synchronizes} with reader threads 3-and finally it \textit{reclaims} the heap location. The mutator thread's reference to the heap location in between \textit{unlinking} and \textit{synchronising} is represented with the type \textsf{unlinked}. The type of the reference in between the end of \textit{synchronisation} and \textit{reclamation} is represented with the type \textsf{freeable}. Synchronisation action \textit{bounds} the mutator thread from reclaiming the \textit{unlinked} heap location immediately by adding it to the \textit{free-list}. The denotation of \textsf{unlinked} asserts that if an \textsf{unlinked} heap location is in our logical \textit{free-list} then there exists a subset of reader threads running simultaneously -- which we call \textit{bounding} threads -- referencing to the heap location. The denotation of \textsf{freeable} asserts that when all bounding threads leave the RCU read-side critical section it is safe for the mutator thread to perform reclamation. A \textit{bounding-thread} can observe an \textsf{unlinked} heap location as \textsf{iterator} during its existence in the \textit{free-list}(Invariant \textsf{Iterators-Free-List} and Invariant \textsf{Readers-In-Free-List}).

\mypar{Publish-Subscribe Guarantee} Concurrent reader threads must not be affected by a \textit{replacing-an-existing-node-with-a-fresh-one} or \textit{linking-a-single-node} action inserting a new node into the linked data structure in such a way that they cannot access to the fresh nodes until they are \textit{published/linked}. Unlike in the case of \textit{Grace-Period-Guarantee} where the reader threads can observe an unlinked heap node as \textsf{iterator} during the \textit{grace-period}, they cannot observe the fresh heap nodes until they are published. As we see in the operational semantics, once a new heap location is allocated it is in type \textsf{fresh}(observed as \textsf{fresh} by the mutator thread). The fresh heap location can only be allocated and referenced by the mutator thread(Invariant \textsf{Fresh-Writes}). The relation between a freshly allocated heap and the rest of the heap is established in such a way that there exists no heap node reaching to the freshly allocated one(Invariant \textsf{Fresh-Reachable}). Moreover, the freshly allocated heap locatation cannot be observed by the reader threads(preserves Invariant \textsf{Readers-Iterator-Only}) until it is published(Invariant \textsf{Fresh-Not-Reader}). Our system enforce \textit{locality} on the effects of linking/replacing a new heap node which we have discussed through Figure \ref{fig:freshframeout} Section \ref{subsection:type-rules}. In the case of replacement the crucial assumption in the framing of the type rule \textsc{T-LinkF} is to have the fields of the fresh node and replaced one pointing the same nodes(preserves Invariant \textsf{Fresh-Points-Iterator}).

\mypar{Memory-Barrier Guarantee} RCU write-side critical section has to guarantee that none of the updates to the memory cause invalid memory accesses. If a heap location is observed as \textsf{iterator} by the mutator thread then it must be observed as \textsf{iterator} by other threads(Invariant \textsf{Writer-Unlink}) and all of the heap locations reaching to the node starting from root can be observed as \textsf{iterator} as well(denotation $\llbracket \mathsf{rcuItr}\,\rho \,\N \rrbracket_{tid}$). Reader threads can only have one type of observation \textsf{iterator}(Invariant \textsf{Readers-Iterator-Only}) for a heap location where this heap location can be updated by the mutator and reader threads referencing on this heap node become \textit{bounding-threads} for the reclamation of the heap location(Denotation $\llbracket \mathsf{rcuItr} \rrbracket_{tid}$ and Invariant \textsf{Readers-In-Free-List}). However, \textit{bounding-threads} are the only ones which can observe the \textsf{unlinked} heap location as \textsf{iterator}: new coming reader threads cannot access to the \textsf{unlinked} heap. There exists links to mutated heap nodes -- e.g. either \textsf{unlinked} or \textsf{freeable} -- only from the heap nodes that are already mutated(Invariant \textsf{Unlinked-Reachability} and Invariant \textsf{Free-List-Reachability}). To sum up, every heap node accessible via traversal starting from the unique root must be valid and observed as \textsf{iterator} both by reader threads and the mutator thread(Invariant \textsf{Readers-Iterator-Only}).

\mypar{RCU Primitives Guaranteed to Execute Unconditionally} Uncoditional execution of RCU Primitives are provided by the definitions in our operational semantics for our RCU primitives(e.g. \lstinline|ReadBegin|, \lstinline|ReadEnd|, \lstinline|WriteBegin|, \lstinline|WriteEnd| and \lstinline|SyncStart;SyncStop|).

\mypar{Guaranteed Read-to-Write Upgrade} Our system provides a clear separation of \textit{traverse-and-update} and \textit{traverse-only} intentions for \textsf{RCU} programming through the type system together with the programming primitives.

\mypar{Acyclic Tree Structure} Additionally, our system also ensures that none of the heap updates introduces \textit{cyclicity}; moreover, violates the \textit{tree} structure(Invariant \textsf{Unique-Reachable}, Invariant \textsf{Unique-Root} and Denotation $\llbracket \mathsf{rcuItr}\,\rho \,\N \rrbracket_{tid}$).
\subsection{Proof}
\label{sec:proof}
...
\begin{figure}[!t]\scriptsize
  \begin{mdframed}
    \begin{flalign*}
      \bullet  = (\bullet_{\sigma},\bullet_O,\cup,\cup) \;\; (F_1 \bullet_F F_2) \overset{\mathrm{def}}{=}  F_1  \cup F_2 \texttt{   when   } dom(F_1) \cap dom(F_2) = \emptyset \\
