...
\mypar{A General Invariant for Read Traversal} All reader threads have to access to a valid heap location even during the \textit{grace-period}. \textsf{Readers-Iterators-Only} invariant asserts that a heap location can only be observed as \textsf{iterator} by the reader threads.

\mypar{Invariants on Grace-Period} We provide invariants related to the \textit{grace-period} via reification of the logical \textit{free-list} in the instrumented state to the \textit{bounding-threads} in the concrete RCU state. \textsf{Readers-In-Free-List} invariant defines the set of \textit{bounding-threads} via asserting the existence of a set of threads referencing to a heap location in the free list. Likewise, we can also think of a \textit{bounding-thread} as the one to which the \textit{grace} is shown. If a reader thread observes a heap location in the \textit{free-list} as \textsf{iterator} then \textsf{Iterators-Free-List} invariant asserts that this thread is not a new comer but one of the \textit{bounding-threads}.

The writer thread can refer to a heap location in the \textit{free-list} either with refernce in type \textsf{freeable} or \textsf{unlinked}. Once the writer unlinks a heap node, it first observes the heap node as \textsf{unlinked} then \textsf{freeable}. The denotation of \textsf{freeable} type distinguishes the end of the \textit{grace-period} via asserting the non-existence of the \textit{bounding-threads} holding references to the referent of the \textsf{freeable}. Unlike the case in the denotation of \textsf{freeable}, the type environment of \textsf{unlinked} asserts that the referent is either in the \textit{free-list} with some \textit{bounding-threads} holding references to it -- the \textit{grace-period} has not finished yet -- or not in the \textit{free-list} -- the \textit{grace-period} has not started yet. 

\mypar{Invariants on Safe Traversal against Unlinking} RCU write-side critical section has to guarantee that none of the updates to the heap cause invalid memory accesses. Our invariants provide reasoning over the mutated heap through possible observations made by the writer thread. \textsf{Writer-Unlink} invariant asserts that a heap location observed as \textsf{iterator} by the writer thread cannot be observed differently by other threads. The denotation of the writer thread's \textsf{rcuItr} reference, $\llbracket \mathsf{rcuItr}\,\rho \,\N \rrbracket_{tid}$, asserts that all the heap locations on the path starting from the root to the referent heap are valid to access and can only be observed as \textsf{iterator}. Both the writer thread and the reader threads observe these heap locations as \textsf{iterator}.

However, unreachability of a mutated heap for the new-coming reader threads is crucial for \textit{soundness}. Only a \textit{bounding-thread} can observe a heap location in the \textit{free-list} as \textsf{iterator}. We introduce this obligation in the the denotation of the reader thread's \textsf{rcuItr} reference, $\llbracket \mathsf{rcuItr} \rrbracket_{tid}$, and it is important in terms of  the preservation of \textsf{Readers-Iterator-Only} invariant for the set of \textit{bounding-threads} against unlinking. \iso{\textsf{Free-List-Reachability} invariant together with \textsf{Unlinked-Reachability} ensure that any heap location which has a field access to another heap location in the \textit{free-list} is also in the \textit{free-list}. To put it another way, the heap locations in the \textit{free-list} form a chain of \textsf{freeable} and \textsf{unlinked} heap locations.}

\mypar{Invariants on Safe Traversal against Inserting/Replacing} Concurrent reader threads must also not be affected by \textit{replacing-an-existing-node-with-a-fresh-one} or \textit{inserting-a-single-fresh-node}. The crucial essence in providing memory safety against these actions is enforcing unreachability of the fresh nodes until they are \textit{published/linked}. Unlike in the case of \textit{unlinking} where the reader threads can observe an unlinked heap node as \textsf{iterator} during the \textit{grace-period}, the reader threads cannot observe the fresh heap nodes until they are published. \textsf{Fresh-Writes} invariant asserts that a fresh heap location can only be allocated and referenced by the writer thread. The relation between a freshly allocated heap and the rest of the heap is established by \textsf{Fresh-Reachable} in such a way that there exists no heap node reaching to the freshly allocated one and this serves to the preservation of the tree structure. Likewise the case for unlinking, \textsf{Readers-Iterators-Only} must be preserved against inserting/replacing a fresh node and \textsf{Fresh-Not-Reader} invariant serves via asserting that the reader threads cannot observe a heap location as \textsf{fresh}.\iso{.}
