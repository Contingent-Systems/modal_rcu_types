...
\paragraph{Corrections to the published invariants.}  The corrections are not cosmetic.  Five of the published invariants are defective.  \textsf{Unique-Reachable} is vacuous as written and admits heaps that are not trees.  \textsf{Readers-Iterators-Only} is contradictory in any state containing both a reader and a freshly allocated node, so it can be used to conclude anything.  \textsf{Fresh-Reachable} is satisfied by its own hypothesis and therefore constrains nothing.  \textsf{Fresh-Points-Iterator} contradicts \textsf{Fresh-Not-Reader}.  And \textsf{Unlinked-Reachability} does not chain: its conclusion is weaker than its hypothesis, so the induction on path length stalls at the first \textsf{freeable} predecessor.  That last one matters most, because the reachability property it is cited for --- that an unlinked node is never reachable from the root --- is what stops a newly arrived reader from reaching a node scheduled for reclamation, and it is exactly what the one-step formula fails to give.
