...
\mypar{Invariants on Grace-Period} We provide invariants related to the \textit{grace-period} via reification of the logical \textit{free-list} in the instrumented state to the \textit{bounding-threads} in the concrete RCU state. In operational semantics the reader threads become \textit{bounding-threads} once the \textit{grace-period} starts. \textsf{Readers-In-Free-List} invariant defines the set of \textit{bounding-set} via asserting existence of a set of threads referencing to a heap location in the free list. We can also think of a \textit{bounding-thread} as the one to which the \textit{grace} is shown. If a reader thread observes a heap location in the \textit{free-list} as \textsf{iterator} then \textsf{Iterators-Free-List} invariant asserts that this thread is one of the \textit{bounding-threads}. Our invariants are generalized to handle \textit{batched-deallocation}. \textsf{Free-List-Reachability} invariant ensures that any heap location which has a heap access to another heap location in the \textit{free-list} is also in the \textit{free-list}. The writer thread can refer to a heap location in the \textit{free-list} either with refernce in type \textsf{freeable} or \textsf{unlinked}. The denotation of \textsf{freeable} type distinguishes the end of the \textit{grace-period} via asserting the non-existence of the \textit{bounding-threads} holding references to the referent of the \textsf{freeable}. Unlike the case in the denotation of \textsf{freeable}, the type environment of \textsf{unlinked} asserts that the referent is either in the \textit{free-list} with some \textit{bounding-threads} holding references to it -- the \textit{grace-period} has not finished yet -- or not in the \textit{free-list} -- the \textit{grace-period} has not started yet. 

\mypar{Invariants on Safe Traversal against Unlinking} RCU write-side critical section has to guarantee that none of the updates to the heap cause invalid memory accesses. Our invariants provide reasoning over the mutated heap through possible observations made by the writer thread. Once the writer unlinks a heap node that it first observes the heap node as \textsf{unlinked} then \textsf{freeable} once the \textit{grace-period} ends. \textsf{Writer-Unlink} invariant asserts that a heap location observed as \textsf{iterator} by the writer thread cannot be observed differently by other threads . The denotation of the writer thread's \textsf{rcuItr} reference, $\llbracket \mathsf{rcuItr}\,\rho \,\N \rrbracket_{tid}$, asserts that all the heap locations on the path starting from the root to the referent heap location are accessible and can only be observed as \textsf{iterator}. However, unreachability of the mutated heap for the new-coming reader threads is crucial. Likewise \textsf{Free-List-Reachability}, \textsf{Unlinked-Reachability} invariant captures the fact that any heap location which has a heap access to a heap location observed as \textsf{unlinked} has already been unlinked or put into the \textit{free-list} by the writer thread. Only a \textit{bounding-thread} in the \textit{free-list} -- which is observed either \textsf{unlinked} or \textsf{freeable} by the writer -- can observe a heap location \textsf{iterator}. This is ensured by the denotation of the reader thread's \textsf{rcuItr}, $\llbracket \mathsf{rcuItr} \rrbracket_{tid}$. It is important to note that this preserves the \textsf{Readers-Iterator-Only} invariant as the set of \textsf{bounding-threads} is a subset of the reader threads.

\mypar{Invariants on Safe Traversal against Inserting/Replacing} Concurrent reader threads must not be affected by \textit{replacing-an-existing-node-with-a-fresh-one} or \textit{inserting-a-single-fresh-node}. The crucial essence in providing memory safety against these actions is enforcing inaccessibility of the fresh nodes until they are \textit{published/linked}. Unlike in the case of \textit{unlinking} where the reader threads can observe an unlinked heap node as \textsf{iterator} during the \textit{grace-period}, they cannot observe the fresh heap nodes until they are published. \textsf{Fresh-Writes} invariant asserts that a fresh heap location can only be allocated and referenced by the writer thread. The relation between a freshly allocated heap and the rest of the heap is established by \textsf{Fresh-Reachable} in such a way that there exists no heap node reaching to the freshly allocated one. As a part of ensuring safe traversals for the reader threads, \textsf{Readers-Iterators-Only} ensures that the readers threads always observe a heap location as \textsf{iterator}. Specifically for the case of inserting/replacing a fresh heap node, and \textsf{Fresh-Not-Reader} asserts that the reader threads cannot observe a heap location as \textsf{fresh}.\iso{add fresh next.}

\mypar{Tree Structure} Additionally, our system also ensures that none of the heap updates introduces \textit{cyclicity}; moreover, violates the \textit{tree} structure. \textsf{Unique-Reachable} invariant asserts that every heap location reachable from root is accessed by following an unique path. To preserve tree structure, textsf{Unique-Root} enforce unreachability of the root from any heap location that is reachable from the root. 
\subsection{Proof}
\label{sec:proof}
...
There are, however, two remaining subtleties to address.  First,
...
\end{lemma}
\begin{proof}
By induction on the typing derivation, appealing to Lemma \ref{lem:axmsoundness} for primitives, Lemma \ref{lem:cntxsubt-m} and consequence for subtyping, and otherwise appealing to structural rules of the Views logic and inductive hypotheses.
