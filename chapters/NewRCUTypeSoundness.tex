...
\mypar{Invariants on Safe Traversal against Unlinking} RCU write-side critical section has to guarantee that none of the updates to the heap cause invalid memory accesses. Our invariants provide reasoning over the mutated heap through possible observations made by the writer thread. Once the writer unlinks a heap node, it first observes the heap node as \textsf{unlinked} then \textsf{freeable} -- \textit{grace-period} ends. \textsf{Writer-Unlink} invariant asserts that a heap location observed as \textsf{iterator} by the writer thread cannot be observed differently by other threads. However, unreachability of the mutated heap for the new-coming reader threads is crucial. The denotation of the writer thread's \textsf{rcuItr} reference, $\llbracket \mathsf{rcuItr}\,\rho \,\N \rrbracket_{tid}$, asserts that all the heap locations on the path starting from the root to the referent heap are valid to access and can only be observed as \textsf{iterator}. Similar to \textsf{Free-List-Reachability}, \textsf{Unlinked-Reachability} invariant captures the fact that any heap location observed as \textsf{unlinked} can be reached by  another heap location which has already been unlinked or put into the put into the \textit{free-list} by the writer thread.  Only a \textit{bounding-thread} in the \textit{free-list} can observe a heap location as \textsf{iterator}. We introduce this obligation in the the denotation of the reader thread's \textsf{rcuItr} reference, $\llbracket \mathsf{rcuItr} \rrbracket_{tid}$. We should note that the set of \textsf{bounding-threads} is a subset of the reader threads and it is important to preserve \textsf{Readers-Iterator-Only} invariant -- the readers threads always observe a heap location as \textsf{iterator} -- against unlinking.

\mypar{Invariants on Safe Traversal against Inserting/Replacing} Concurrent reader threads must also not be affected by \textit{replacing-an-existing-node-with-a-fresh-one} or \textit{inserting-a-single-fresh-node}. The crucial essence in providing memory safety against these actions is enforcing unreachability of the fresh nodes until they are \textit{published/linked}. Unlike in the case of \textit{unlinking} where the reader threads can observe an unlinked heap node as \textsf{iterator} during the \textit{grace-period}, the reader threads cannot observe the fresh heap nodes until they are published. \textsf{Fresh-Writes} invariant asserts that a fresh heap location can only be allocated and referenced by the writer thread. The relation between a freshly allocated heap and the rest of the heap is established by \textsf{Fresh-Reachable} in such a way that there exists no heap node reaching to the freshly allocated one and this serves to the preservation of the tree structure. Likewise the case for unlinking, \textsf{Readers-Iterators-Only} must be preserved against inserting/replacing a fresh node and \textsf{Fresh-Not-Reader} invariant serves via asserting that the reader threads cannot observe a heap location as \textsf{fresh}.

\mypar{Tree Structure} Our invariants enforce the \textit{tree} structure heap layouts for data structures. \textsf{Unique-Reachable} invariant asserts that every heap location reachable from root can only be reached with following an unique path. To serve for preservation of the tree structure, \textsf{Unique-Root} enforces unreachability of the root from any heap location that is reachable from it. 
\subsection{Proof}
\label{sec:proof}
