...
\label{sec:lemmas}
In this section we summarize the important global invariants imposed on instrumented states in the proof.
 %RCU model which we introduce formally as a conjuction of \textit{global invariants} in 
Full details appear in Appendix \ref{sec:memaxioms}.
% and the type denotations. 
We aim to convey an intiution that our system captures the requirements of RCU setting~\cite{abssem}.

\mypar{Grace-Period Guarantee} To reclaim a heap location, a mutator thread must synchronize with all of the reader threads with overlapping read-side critical sections.
...
%The mutator thread's reference to the heap location in between {unlinking} and {synchronising} is represented with the type \textsf{unlinked}. The type of the reference in between the end of \textit{synchronisation} and \textit{reclamation} is represented with the type \textsf{freeable}. 
...
%Synchronisation action \textit{bounds} the mutator thread from reclaiming the \textit{unlinked} heap location immediately by adding it to the \textit{free-list}. 
The denotation of \textsf{unlinked} asserts that if an \textsf{unlinked} heap location is in our logical \textit{free-list} then
it is unreachable via the heap, but possibly accessible to other threads via local variables.
 %there exists a subset of reader threads running simultaneously -- which we call \textit{bounding} threads -- referencing to the heap location.
The denotation of \textsf{freeable} --- only possible after synchronizing --- requires that the object is not referenced from the heap or other threads.
%A \textit{bounding-thread} can observe an \textsf{unlinked} heap location as \textsf{iterator} during its existence in the \textit{free-list}(Invariant \textsf{Iterators-Free-List} and Invariant \textsf{Readers-In-Free-List}).
