\section{Soundness}
\label{sec:soundness}
This section outlines the proof of type soundness -- our full proof appears in Appendices \ref{sec:prooflemmas}, \ref{sec:memaxioms}, \ref{lem:lematom} and \ref{lem:lemstructural} of the technical report~\cite{isotek}.
...
\begin{enumerate}
    ...
    The instrumented execution states are also constrained by additional \textit{global} \textsf{WellFormed}ness \textit{invariants} -- see Appendix \ref{sec:memaxioms} of the technical report~\cite{isotek} for detailed formal invariants -- the type system is intended to maintain, such as tree structure of the data structure.
...
The statements below are informal; Appendix \ref{sec:inv-revised} gives each one in full and in corrected form, with a record of what changed and why.  Appendix \ref{sec:memaxioms} retains the statements as originally published.
...
Moreover, the denotation of \textsf{rcuFresh} type, $\llbracket \mathsf{rcuFresh}\,\N \rrbracket_{tid}$, enforces that fields in $\N$ point to valid heap locations (observed as \textsf{iterator} by the writer thread), and that every \textsf{RCU} field \emph{outside} $dom(\N)$ is \textsf{null}.  The latter clause makes $\N$ determine the fresh node's entire \textsf{RCU} footprint rather than only the part of it that has been written.  It is an invariant of the fresh-node lifecycle rather than a new obligation: \textsc{T-Alloc} establishes it, since a freshly allocated object has all fields \textsf{null} and an empty field map, and \textsc{T-WriteFH} --- the only rule that writes a field of a \textsf{rcuFresh} object --- preserves it, since it moves exactly the field it writes into $dom(\N)$.  \textsc{T-Insert} needs it: without it an untracked field of the fresh node could point anywhere, and the inserted node could acquire a second child.
...
\paragraph{Corrections to the published invariants.}  The corrections are not cosmetic.  Six of the published invariants are defective.  \textsf{Unique-Reachable} is vacuous as written and admits heaps that are not trees.  \textsf{Readers-Iterators-Only} is contradictory in any state containing both a reader and a freshly allocated node, so it can be used to conclude anything.  \textsf{Fresh-Reachable} is satisfied by its own hypothesis and therefore constrains nothing.  \textsf{Fresh-Points-Iterator} contradicts \textsf{Fresh-Not-Reader}.  And \textsf{Unlinked-Reachability} does not chain: its conclusion is weaker than its hypothesis, so the induction on path length stalls at the first \textsf{freeable} predecessor.  And \textsf{Heap-Domain-Closure} is false as stated: it constrains every edge in the heap, but an unlinked node retains a pointer to its old child and nothing orders the reclamation of two unlinked nodes, so a well-typed section may free a node that another, itself awaiting reclamation, still points at.  The pointer dangles harmlessly, in a node that is detached and unreachable --- which is exactly the exemption \textsf{No-Sharing} already carries and this one does not.

Two of these matter most.  \textsf{Unlinked-Reachability} is cited for the property that an unlinked node is never reachable from the root, which is what stops a newly arrived reader from reaching a node scheduled for reclamation, and the one-step formula does not give it.  \textsf{Heap-Domain-Closure} is the invariant the \texttt{Free} case rests on, and under the published statement that case cannot be proved at all.

A sixth defect lies not in an invariant but in two of the type rules.  Neither \textsc{T-Replace} nor \textsc{T-UnlinkH} prevents a \textsf{rcuFresh} reference in the environment from having a field pointing at the node the rule unlinks.  Unlinking costs that node its \textsf{iterator} observation, so the resulting state violates \textsf{Fresh-Points-Iterator} and also the denotation of the fresh reference's own type; linking such a reference in afterwards would splice a node already scheduled for reclamation back into the structure.  The aliasing premises of both rules quantify only over \textsf{rcuItr} references, and the denotation of a type environment is an intersection rather than a separating conjunction, so nothing else excludes it.  Both rules now carry a premise that does.

Two of the six were found by attempting the mechanization rather than by reading, and both are cases whose proof the technical report calls trivial.  Every defect is accompanied by a machine-checked proof or counterexample, and the corrected set is itself machine-checked satisfiable, so the repairs are not vacuously strong.  The same development discharges the parts of the argument that concern reachability rather than ghost state: that each of the three heap-mutating rules preserves the tree structure, that heap-domain closure survives every action touching the heap --- including \texttt{Free}, the one case where it does not hold automatically --- and that the corrected \textsf{Unlinked-Reachability} and \textsf{Free-List-Reachability} chain.  It is axiom-free.
...
\end{lemma}
\begin{proof}
In Appendix \ref{sec:prooflemmas} Lemma \ref{lemma:stblwap} of the technical report~\cite{isotek}.
\end{proof}
We elide the statement of the analogous result for the read-side critical section, available in Appendix \ref{sec:prooflemmas} of the technical report.
...
\end{lemma}
\begin{proof}
By case analysis on $\alpha$. Details in Appendix \ref{sec:prooflemmas} of the techical report~\cite{isotek}. % Lemma \ref{lem:axmsoundnessap}
\end{proof}
...
Full details in Appendix \ref{sec:prooflemmas} of the technical report~\cite{isotek}. %Lemma \ref{lemma:embedwap}.
  \end{proof}
