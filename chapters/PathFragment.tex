...
%
% Companion to prototype/path_domain.h, which implements everything below and
...
% \defn and \RCU are also defined by InvariantsRevised; use \providecommand so
% that this chapter compiles both on its own and alongside it.
\newcommand{\pvar}{\pi}
\newcommand{\MayAlias}{\textsf{MayAlias}}
\providecommand{\defn}{\;\triangleq\;}
\providecommand{\RCU}{\textsf{RCU}}
...
The fragment is implemented, and so is a checker built on it.  Four layers, in
dependency-free C++17 except the last, passing 86 unit and property checks:
\texttt{path\_domain.h} for the material of this section --- CNF membership,
\MayAlias{}, join, substitution, reindexing, widening and the loop fixpoint;
\texttt{rcu\_types.h} for types, field maps and environments;
\texttt{rcu\_rules.h} for the type rules as transfer functions; and
\texttt{rcu\_check.h} for a statement IR and the forward dataflow over it.

\texttt{frontend.cpp} is the only part that needs a compiler.  It translates a C
function into that IR through Clang's CFG, taking back edges from the dominator
tree and refinements from the branch conditions, and reports what the dataflow
finds against the source.  It needs no annotation language: \texttt{\_\_rcu} on
a field is enough to recognise the reads and the writes, which are taken from
the field's type rather than from an accessor macro, so
\texttt{rcu\_dereference} and \texttt{rcu\_assign\_pointer} need not appear at
all; \texttt{rcu\_read\_lock}/\texttt{rcu\_read\_unlock},
\texttt{synchronize\_rcu} and the \texttt{kfree} family distinguish every
remaining action the type system has a rule for.  Only the root has to be
marked, since nothing in the source distinguishes it.

\paragraph{On the paper's own example.}  \texttt{testdata/bst\_full.c} is the
delete of Appendix \ref{appendix:bst_del} written as C: the traversal, the
no-right-child case, and the two-child case that allocates a copy of the
successor, splices it over the node and reclaims the original after a grace
period.  It type checks.

That it type checks is only worth reporting alongside what it rejects, so the
same file carries the two-child case with one field left out of the copy, and
the checker rejects it --- ``the fresh node's field map differs from the one it
replaces, so it would not mirror it''.  That is the side condition
\S\ref{sec:inv-open} adds to \textsc{T-Replace}, refusing an incomplete copy in
C rather than in a proof.  Removing the grace period from the same function is
likewise rejected, at the \texttt{free}.

\paragraph{What it does not do.}  A function is checked on its own, with its
pointer parameters assumed to be iterators; summaries carry kinds across a call
but not paths.  The limit is visible in the same example: the delete checks when
it reads its cursor out of the root, and does not when the two cursors arrive as
parameters, because nothing at the boundary says the second is the first's
child.  Carrying that relationship is the interprocedural form of what
\S\ref{sec:pathfrag-def} does within a function, and is not attempted.
...
\subsection{Exactness of Stage 2}
\label{sec:pathfrag-exact}

The completeness question left open above is settled here, in the negative for
CNF as defined and positively under one further condition.

\paragraph{CNF alone is not enough.}  Take $\Fields = \{a,b\}$, one variable
$\pvar : \{a,b\}$, and
\[\rho = \pvar\cdot a \qquad \rho' = b\cdot\pvar\]
Both are in CNF --- each mentions $\pvar$ once, and vacuously in increasing
order.  Stage 1 consumes nothing, since the first segments differ.  Stage 2
relaxes both occurrences independently and intersects $D^{*}a$ with $bD^{*}$,
which is non-empty, so the procedure answers \textsf{yes}.  But
$\MayAlias(\rho,\rho')$ asks for a single $w$ with $wa = bw$, and induction on
$|w|$ gives none when $a \neq b$.  So the procedure is strictly
over-approximating on CNF input.  What CNF's increasing-order condition rules
out is \emph{cross}-variable misalignment, $\pvar\cdot a$ against
$b\cdot\pvar'$; it says nothing about one variable appearing at two different
depths, which is what this is.

\paragraph{The exact characterisation.}  Imprecision has exactly one source.

\begin{theorem}[Exactness]
\label{thm:alias-exact}
If the Stage 1 remainders share no path variable, Stage 2 is exact.
\end{theorem}
\begin{proof}
By CNF a variable occurs at most once per path, so a variable surviving Stage 1
in only one remainder occurs exactly once in the relaxed problem.  For such a
variable, replacing $\pvar : D$ by $D^{*}$ discards nothing: the constraint it
drops is that two occurrences agree, and there is only one.  Relaxation is then
literally the quantifier $\exists\theta(\pvar) \in D^{*}$ that $\MayAlias$ asks
for, so the intersection is non-empty exactly when some valuation makes the two
languages meet.  Stage 1 is exact by Lemma \ref{lem:alias-sound}, and the
composition of two exact stages is exact.
\end{proof}

\begin{definition}[Anchored CNF]
\label{def:anchored}
A CNF environment is \emph{anchored} when, for every variable $\pvar$, all paths
mentioning $\pvar$ agree segment-for-segment on the segments strictly preceding
it.
\end{definition}

\begin{corollary}
\label{cor:anchored-exact}
On anchored environments $\MayAlias$ is exact.
\end{corollary}
\begin{proof}
If $\rho$ and $\rho'$ both mention $\pvar$ they agree on everything before it, so
Stage 1's maximal identical prefix reaches $\pvar$, and, the segments there being
identical, consumes it too.  No shared variable survives, and
Theorem \ref{thm:alias-exact} applies.
\end{proof}

Anchoring is the syntactic form of what a traversal actually produces: the
cursors of a loop all carry the loop word at the same position, because they were
all reindexed by the same back edge.  It is decidable by inspection --- for each
variable, compare the segments preceding it in every path that mentions it ---
and \texttt{inAnchoredCNF} in the prototype does exactly that, so a checker can
accept the class on which it is exact rather than settling for soundness.  Every path in the paper is anchored, which
is why the unit tests are exact, and the counterexample above is not anchored ---
$\pvar$ sits at depth $0$ in one path and depth $1$ in the other.

\paragraph{Measured.}  Over $10^{5}$ random CNF pairs across five seeds,
partitioned by whether a variable survives Stage 1 in both remainders: where
none does, the procedure agrees with brute-force enumeration in $93{,}564$ of
$93{,}564$ cases; where one does, in $4{,}142$ of $6{,}436$, or $64\%$.  No
answer was unsound in either class.  Separately, over a further $10^{5}$ pairs,
it is exact on all $90{,}256$ that are anchored.  So the residual
imprecision reported in \S\ref{sec:pathfrag-eval} is not spread thinly over the
domain --- it is entirely inside the non-anchored class, and a checker that
rejects non-anchored environments outright is exact rather than
merely sound.
