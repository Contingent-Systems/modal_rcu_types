...
dependency-free C++17 except the last, passing 84 unit and property checks:
...
a field, and the existing \texttt{rcu\_dereference}, \texttt{rcu\_assign\_pointer},
\texttt{synchronize\_rcu} and \texttt{kfree}, already distinguish every action
the type system has a rule for.  Only the root has to be marked, since nothing
in the source distinguishes it.
...
\paragraph{Measured.}  Over $30{,}000$ random CNF pairs, partitioned by whether a
variable survives Stage 1 in both remainders: where none does, the procedure
agrees with brute-force enumeration in $27{,}148$ of $27{,}148$ cases; where one
does, in $43.5\%$.  No answer was unsound in either class.  So the residual
