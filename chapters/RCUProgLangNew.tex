...
It is worth being explicit about where $k$ is bound, since the notation invites the wrong reading.  An index is quantified at the level of the \emph{whole type environment}, not within an individual type: writing $\rho.f^{k}$ in one type and $\rho.f^{k}.f$ in another asserts the existence of a single $k$ making both accurate simultaneously, which is exactly what expresses that the two references share a path prefix.  Were each occurrence quantified separately the types would still be individually true and the relationship between the references --- the only thing the index is there to carry --- would be lost.  A consequence is that indices do not interact with subtyping: the subtyping judgment relates two types under a fixed environment, so the index is already bound when it applies, and no rule needs to introduce, eliminate, or match indices.
\looseness=-1
...
\N_1 = \N_2 \and
\forall_{f'} \ldotp \textsf{FType}(f')=\textsf{RCU} \implies f' \in dom(\N_1) \\
...
Finally, the rule must ensure that no types in $\Gamma$ are invalidated.  This could happen one of three ways: either a field map in $\Gamma$ for an alias of $x$ duplicates the assumption that $x.f_1==z$ (which is changed by this write), or $\Gamma$ contains a descendant of $r$, whose path from the root will change when its ancestor is modified, or $\Gamma$ contains a \textsf{rcuFresh} reference one of whose fields points at $z$.  The last is what the final premise excludes.  It is needed because unlinking costs $z$ its \textsf{iterator} observation, while the denotation of \textsf{rcuFresh} requires every field target to carry one; without the premise a well-typed program can reach a state in which a fresh node points at an unlinked one, and linking that fresh node in later would splice a node already scheduled for reclamation back into the structure.  The \textsf{rcuItr} implication above does not cover this case, since a \textsf{rcuFresh} reference is not an \textsf{rcuItr}, and the environment does not either, as $\llbracket \Gamma \rrbracket$ is an intersection rather than a separating conjunction.
The final assumption of \textsc{T-UnlinkH} (the implication)
checks that for every \textsf{rcuItr} reference $n$ in $\Gamma$, it is not a path alias of $x$, $z$, or $r$; no entry of its field map ($m$) refers to $r$ or $z$ (which would imply $n$ aliased $x$ or $z$ initially); and its path is not an extension of $r$ (i.e., it is not a descendant). \textsf{MayAlias} is a predicate on two paths (or a path and set of paths) which is true if it is possible that any concrete paths the arguments may abstract (e.g., via adding non-determinism through $|$ or abstracting iteration with indexing) \emph{could} be the same.  The negation of a \textsf{MayAlias} use is true only when the paths are guaranteed to refer to different locations in the heap.

Every framing side condition in this figure is a negated \textsf{MayAlias} query, so whether they can be discharged mechanically is the question of whether \textsf{MayAlias} is decidable.  On the path language as written it is not obviously so --- abstracted iteration makes the general problem one of word equations --- but on a fragment corresponding to the traversals a critical section actually performs it is, and cheaply: linear in the paths after a prefix reduction, then a product construction.  Appendix \ref{sec:pathfrag} gives that fragment, the decision procedure, the companion algorithms for control-flow merges and loop back edges, and the condition under which the procedure is not merely sound but exact.  Checking a critical section is then a terminating forward dataflow, which is what a compiler front end can run; a prototype of the path domain accompanies it.
...
Preserving the tree structure needs slightly more than that equality, which is why the rule also requires $dom(\N_1)$ to cover every \textsf{RCU} field.  Field maps record only the fields that have been read or written, so $\N_1 = \N_2$ on its own says the two nodes agree wherever \emph{both} are tracked, and leaves an untracked \textsf{RCU} field of the replaced node free to differ --- which would move a subtree rather than preserve it.  With the side condition, $\N_1$ and $\N_2$ between them pin the whole \textsf{RCU} footprint of both nodes, and the replacement's subtree really is the replaced node's subtree.  The condition is discharged by any code that copies the node it replaces: writing a field of the fresh node with \textsc{T-WriteFH} requires that field to be in the replaced node's map already, so a replacement that copies every \textsf{RCU} field satisfies it by construction.  The binary search tree deletion of Appendix \ref{appendix:bst_del} does exactly this, setting both \texttt{Left} and \texttt{Right} of the fresh node before the replacement.
\textsc{T-Replace} carries one further premise, mirroring the last premise of \textsc{T-UnlinkH} and needed for the same reason: no \textsf{rcuFresh} reference in $\Gamma$ may have a field pointing at $o$, the node being replaced.  Replacement unlinks $o$, which costs it its \textsf{iterator} observation, and the denotation of \textsf{rcuFresh} requires every field target to carry one.  The binary search tree deletion satisfies this too --- the fresh node it builds points at the children of the node being replaced, never at the node itself.
...
\textsc{T-Insert} type checks heap updates that link a fresh node into a linked data structure. Inserting a \textsf{rcuFresh} reference also faces some of the aliasing complications that we have already discussed for direct unlinking and replacing a node. Unlike the replacement case, the path to the last heap node (the referent of $o$) from the root is \emph{extended} by $f$, which risks falsifying the paths for aliases and descendants of $o$. The final assumption(the implication) of \textsc{T-Insert} checks for this inconsistency.
...
We introduce the \textit{syntactic sugaring} $\textsf{RCUWrite } x.f \textsf{ as } y \textsf{ in } \{C\}$ for write-side critical sections where the analogous syntactic sugaring can be found for read-side critical sections in Appendix \ref{appendix:readtypes} of the technical report~\cite{isotek}.
