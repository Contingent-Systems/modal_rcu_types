...
\left\{\begin{array}{l}
(\neg\mathsf{MayAlias}(\rho_3,\{\rho_1,\rho_2\})) \arcr
\land (\forall_{\rho_4\neq \epsilon} \ldotp \neg\mathsf{MayAlias}(\rho_3, \rho_2.\rho_4) )
\end{array}\right.
\end{array}
}
...
\textsc{T-LinkFL} type checks heap updates that links a fresh node without doing replacement from the data structure. Linking a \textsf{rcuFresh} reference without doing replacement faces the some of the aliasing complications that we have already discussed for direct unlinking and replacing an existing node with a fresh one. There is also another rule, \textsc{T-LinkF-Null}, not shown in Figure \ref{fig:tss}, which handles the case where the fields of the fresh node are not object references, but instead all contain null (e.g., for appending to the end of a linked list or inserting a leaf node in a tree).
