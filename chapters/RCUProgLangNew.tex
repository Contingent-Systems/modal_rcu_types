...
\paragraph{freeable} is the type given to references pointing at unlinked heap location which is safe to reclaim because it is known that no concurrent readers hold references to it.
...
Assertions for \texttt{add} function in lines 19 and 20 in Figure \ref{fig:rculist} show the \textit{loop}'s effects on paths of iterator references used inside the loop, \texttt{cur} and \texttt{par}. 
...
{
 \Gamma\, , y : \textsf{rcuItr}\, \_ \vdash_M C \dashv \Gamma' \;\; \textsf{FType}(f)=\textsf{RCU} \;\;  \textsf{NoFresh}(\Gamma') \;\; \textsf{NoUnlinked}(\Gamma')  \;\; \textsf{NoFreeable}(\Gamma')
}
{
