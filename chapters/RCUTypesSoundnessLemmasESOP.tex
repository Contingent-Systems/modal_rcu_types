...
\end{case}
\begin{case} - \textbf{FPI} \emph{This case does not go through; see the
corresponding case of Lemma \ref{lemma:linkf}.}  \textsc{T-UnlinkH} makes $o_y$
\textsf{unlinked}, which costs it $\textsf{iterator}\,\sigma.l$ by
\textbf{WULK}, and \ref{ahu6} constrains only $\textsf{rcuItr}$ variables, so a
$\textsf{rcuFresh}$ variable in $\Gamma$ with a field pointing at $o_y$ makes
\textbf{FPI} --- and the denotation of that variable's own type --- fail in
$\sigma'$.  The repair is to extend \ref{ahu6} to fresh variables with $y \neq
z$.
\end{case}
...
\end{case}
\begin{case} - \textbf{FPI} \emph{This case does not go through, and the rule
...
$\textsf{iterator}\,\sigma.l$.  So if any location observed as \textsf{fresh}
has a field pointing at $o_r$, \textbf{FPI} holds in $\sigma$ and fails in
$\sigma'$.

Nothing excludes that.  The rule's aliasing premise \ref{ahu6f} ranges over
$x:\textsf{rcuItr}\,\rho''\,\N'''([f'\rightharpoonup y])$ and concludes $y \neq
r$; a variable typed $\textsf{rcuFresh}$ is not an $\textsf{rcuItr}$, so it is
not covered.  Nor does the environment's shape help: $\llbracket \Gamma
\rrbracket$ is the \emph{intersection} of the individual variables' denotations,
not a separating conjunction, so two variables may denote overlapping structure.
And the state is reachable: \textsc{T-WriteFH} sets a fresh node's field to an
$\textsf{rcuItr}$ target, which is exactly what $r$ is until the replacement.

The failure is not confined to \textbf{FPI}.  $\llbracket
\textsf{rcuFresh}\,\N\rrbracket$ itself requires every target in $codom(\N)$ to
be observed as \textsf{iterator}, so such a $\Gamma$ fails the denotation of its
own post-environment and this lemma is false as stated.  What it guards against
is concrete: linking that fresh node in afterwards would splice a node already
scheduled for reclamation back into the structure.

The repair is to extend \ref{ahu6f} to fresh variables,
\[\forall_{x \in \Gamma, \N''', f', y} \ldotp
  (x:\textsf{rcuFresh}\,\N'''([f'\rightharpoonup y])) \implies y \neq r\]
after which the case is immediate: no \textsf{fresh} node points at $o_r$, every
other \textsf{fresh} node's targets are untouched, and \textbf{FPI} is inherited
from \ref{ahu16f}.\textbf{FPI}.  The counterexample is machine checked as
\texttt{FPI\_not\_preserved\_by\_replace} in \texttt{rocq/WellFormed.v}.
\textsc{T-UnlinkH} has the same defect for the same reason and takes the same
repair with $y \neq z$; \textsc{T-Insert} is unaffected, since it unlinks
nothing and so costs no location its \textsf{iterator} observation.
\end{case}
