...
&\land (\forall_{g} \ldotp \textsf{FType}(g)=\textsf{RCU} \land g \notin dom(\N) \implies h(s(x,tid),g) = \textsf{null})
\end{array}
\right\}
\\
...
\textsf{root} \in O(rt) \land s(x,tid) \notin dom(F) ) \end{array}
\end{array}
\right\}
\end{array}
\]
...
\emph{These figures are superseded by Appendix \ref{sec:inv-revised}, which
states each invariant in corrected form and records what changed and why.  Six
of the versions below --- \textbf{UNQR}, \textbf{RITR}, \textbf{FR},
\textbf{FPI}, \textbf{ULKR} and \textbf{HD} --- are defective in ways that are
machine checked: vacuous, unsatisfiable, too weak to support the uses the proofs
make of them, or false outright.  They are retained here as the published statements; where a proof case
below appeals to an invariant by name, it is the corrected version of Appendix
\ref{sec:inv-revised} that is meant.}
\begin{enumerate}
...
\forall_{f\in dom(\N')} \ldotp f\neq f_2 \implies  \N'(f) = \textsf{null}
      \label{ahu5} \\
\forall_{w \in \Gamma, \N_4, f_3, m'} \ldotp (w:\textsf{rcuFresh}\,\N_4([f_3\rightharpoonup m'])) \implies m' \neq z
      \label{ahu5b} \\
...
\begin{case}\label{unqr} - \textbf{UNQR}
Let $o_x$, $o_y$ and $o_z$ be $\sigma.h^{*}(\sigma.rt,\rho)$,
$\sigma.h^{*}(\sigma.rt,\rho.f_1)$ and $\sigma.h^{*}(\sigma.rt,\rho.f_1.f_2)$
respectively.

\textbf{UNQR} quantifies over \emph{every} location reachable from the root, so
checking that the three paths in the footprint remain distinct is not sufficient:
the write $x.f_1 := r$ shortens by one segment every path that passed through the
rewritten edge, and the obligation is that no such path collides with any other.
The argument is the general one about a single field update, mechanized as
\texttt{UNQR\_unlink} (and \texttt{UNQR\_unlink\_reachable}, which discharges its
side condition from reachability) in \texttt{rocq/HeapPaths.v}, over the same
$h^{*}$ on a raw heap used here.  It needs only the following, all of which the
rule and the precondition supply.

\begin{enumerate}
\item \textbf{UNQR} holds before the write --- \ref{ahu16}.\textbf{UNQR}.
\item $o_x$ is reachable, at $\rho$ --- denotation of $x:\textsf{rcuItr}\,\rho\,\N$
  in \ref{ahu11}.
\item $\sigma.h(o_x,f_1) = o_y$ and $\sigma.h(o_y,f_2) = o_z$ --- \ref{ahu4}
  ($\N(f_1) = z$ and $\N'(f_2) = r$) with the denotations of $x$ and $z$.
\end{enumerate}

Root is unchanged, $\sigma.rt = \sigma'.rt$, by operational semantics.  Two things
are worth recording.  First, unlinking needs \emph{fewer} hypotheses than
replacement or insertion: there is no fresh node, so nothing to mirror and no
fields-are-null condition.  In particular \ref{ahu5}
($\forall_{f\in dom(\N')}\ldotp f\neq f_2 \implies \N'(f) = \textsf{null}$) plays
no part here --- it bounds how much of the structure becomes unreachable, which is
\textbf{ULKR}'s business, not \textbf{UNQR}'s.  Second, the case that has to be
excluded is a collision between a path left \emph{unchanged} by the write and one
that was shortened by it; ruling it out is why the mechanized characterisation
records that an unchanged path does not traverse the rewritten edge, and it is the
step the path-distinctness argument alone does not reach.
\end{case}
...
\end{case}
\begin{case} - \textbf{FPI}
\textsc{T-UnlinkH} makes $o_y$ \textsf{unlinked}, which costs it
$\textsf{iterator}\,\sigma.l$ by \textbf{WULK}, so as in the corresponding case
of Lemma \ref{lemma:linkf} everything turns on no \textsf{fresh} node pointing
at $o_y$.  That is \ref{ahu5b}.  It does not follow from \ref{ahu6}, which
constrains only $\textsf{rcuItr}$ variables, and without it both \textbf{FPI}
and the denotation of the offending variable's own $\textsf{rcuFresh}$ type fail
in $\sigma'$.  With it the case is immediate, exactly as for
Lemma \ref{lemma:linkf}.
\end{case}
...
\N' = \N''
\label{ahu5f} \\
\forall_{f''} \ldotp \textsf{FType}(f'')=\textsf{RCU} \implies f'' \in dom(\N')
\label{ahu5bf} \\
\forall_{w \in \Gamma, \N_4, f_3, y'} \ldotp (w:\textsf{rcuFresh}\,\N_4([f_3\rightharpoonup y'])) \implies y' \neq r
\label{ahu5cf} \\
\begin{aligned}
...
Let $o_p$ be $\sigma.s(p,tid)$, $o_r$ be $\sigma.s(r,tid)$ and $o_n$ be
$\sigma.s(n,tid)$, so that $o_p$ and $o_r$ are $\sigma.h^{*}(\sigma.rt,\rho)$ and
$\sigma.h^{*}(\sigma.rt,\rho.f)$.

\textbf{UNQR} quantifies over \emph{every} location reachable from the root, so it
is not enough to check that the paths in the footprint stay distinct: the write
$p.f := n$ can affect any path that passes through the rewritten edge.  The
argument is the general one about a single field update, mechanized as
\texttt{UNQR\_replace} in \texttt{rocq/HeapPaths.v} over the same $h^{*}$ on a raw
heap used here.  Its hypotheses are discharged as follows.

\begin{enumerate}
\item \textbf{UNQR} holds before the write --- \ref{ahu16f}.\textbf{UNQR}.
\item $\sigma.h(o_p,f) = o_r$ --- \ref{ahu4f} ($\N(f) = r$) with the denotation of
  $p:\textsf{rcuItr}\,\rho\,\N$ in \ref{ahu11f}.
\item $o_n$ agrees with $o_r$ on every \textsf{RCU} field.  This is where
  \ref{ahu5f} ($\N' = \N''$) alone is too weak, since a field map constrains only
  $dom(\N)$.  With the side condition \ref{ahu5bf}, $dom(\N')$ exhausts the
  \textsf{RCU} fields, so $\N' = \N''$ together with the denotations of
  $r:\textsf{rcuItr}\,\rho'\,\N'$ and $n:\textsf{rcuFresh}\,\N''$ pins
  $\sigma.h(o_r,f_i)$ and $\sigma.h(o_n,f_i)$ to the same location at every such
  $f_i$, which is the mirroring the mechanized proof requires.
\item $o_n$ is unreachable from the root, and in particular $o_n \neq o_p$ ---
  \ref{ahu16f}.\textbf{FR}, whose corrected form says outright that no heap edge
  targets the freshly allocated $o_n$; a location that nothing points to and that
  is not the root cannot be reached.  This step is mechanized as
  \texttt{no\_incoming\_unreachable}.
\item $o_p$ is not reachable from $o_r$.  This is the tree shape rather than a new
  assumption: $o_p$ is reached at $\rho$ and $o_r$ at $\rho.f$, so a path from
  $o_r$ back to $o_p$ would give $o_p$ two distinct paths from the root and
  contradict \ref{ahu16f}.\textbf{UNQR}.  Mechanized as
  \texttt{UNQR\_no\_back\_edge}.
\end{enumerate}

Root is unchanged, $\sigma.rt = \sigma'.rt$, by operational semantics.  Replacement
is the mildest of the three mutations for \textbf{UNQR}: because $o_n$ mirrors
$o_r$, paths through the rewritten edge are preserved in length with $o_n$
substituted for $o_r$, so no unchanged path can collide with a rewritten one and
the case analysis closes without the exclusion that \textsc{Insert} and
\textsc{UnlinkH} need.
\end{case}
...
\end{case}
\begin{case} - \textbf{FPI}
\textbf{FPI} requires every \textsf{RCU} field of a \textsf{fresh} node to point
at a location the writer observes as \textsf{iterator}.  The action makes $o_r$
\textsf{unlinked}, and by \textbf{WULK} it therefore loses
$\textsf{iterator}\,\sigma.l$; so the case turns entirely on no \textsf{fresh}
node pointing at $o_r$.

That is exactly \ref{ahu5cf}, and it is why the rule carries that premise.  It
is not implied by \ref{ahu6f}, which ranges only over
$x:\textsf{rcuItr}\,\rho''\,\N'''([f'\rightharpoonup y])$ --- a
$\textsf{rcuFresh}$ variable is not an $\textsf{rcuItr}$ --- nor by the shape of
the environment, since $\llbracket \Gamma \rrbracket$ is the \emph{intersection}
of the individual variables' denotations rather than a separating conjunction,
so two variables may denote overlapping structure.  Without it the lemma is
false: \textsc{T-WriteFH} sets a fresh node's field to an $\textsf{rcuItr}$
target, which is what $r$ is until the replacement, and the resulting state
satisfies \textbf{FPI} before the write and violates it after
(\texttt{FPI\_not\_preserved\_by\_replace} in \texttt{rocq/WellFormed.v}).  The
same state also fails $\llbracket \textsf{rcuFresh}\,\N''\rrbracket$ itself,
which requires every target in $codom(\N'')$ to be observed as
\textsf{iterator}, so the premise is needed for the post-environment's own
denotation and not only for \textbf{FPI}.

Given \ref{ahu5cf} the case is immediate: no \textsf{fresh} node points at
$o_r$, the targets of every other \textsf{fresh} node are untouched by the
write, and \textbf{FPI} is inherited from \ref{ahu16f}.\textbf{FPI}.
\textsc{T-Insert} needs no such premise, since it unlinks nothing and so costs
no location its \textsf{iterator} observation.
\end{case}
...
\end{case}
\begin{case} - \textbf{WF} Trivial.
\end{case}
\begin{case} - \textbf{FR}
Not quite trivial, though short.  The action \emph{does} create an edge to
$o_n$, namely $\sigma'.h(o_p,f) = o_n$, so if $o_n$ were still observed as
\textsf{fresh} in $\sigma'$ the first clause of \textbf{FR} would fail outright.
It is not: the post-environment types $n$ as $\textsf{rcuItr}\,\rho'\,\N''$, and
by \ref{phu5f} the observation changes from \textsf{fresh} to
$\textsf{iterator}\,tid$ in the same step, so \textbf{FR}'s hypothesis no longer
applies to $o_n$.  For any other location observed as \textsf{fresh} in
$\sigma'$, the action adds no edge targeting it and no stack alias to it, so
both clauses are inherited from \ref{ahu16f}.\textbf{FR}.  The corrected
conjunctive form is what makes this argument available --- the published
disjunction is satisfied by its own hypothesis, so it would carry no
information here.
\end{case}
\begin{case} - \textbf{HD}
The action writes $o_n$ into $o_p.f$ and changes no other field, so the only new
edge is $\sigma'.h(o_p,f) = o_n$ and \textbf{HD} reduces to $o_n \in
dom(\sigma'.h)$.  That holds because $n$ is typed $\textsf{rcuFresh}\,\N''$ in
\ref{ahu11f}, and the denotation of $\textsf{rcuFresh}$ places
$\sigma.s(n,tid)$ in the heap; a field write does not shrink the domain, so it
is still there in $\sigma'$.  Every other edge is inherited from $\sigma$, where
\ref{ahu16f}.\textbf{HD} applies.  Mechanized as \texttt{HD\_h\_upd} in
\texttt{rocq/HeapPaths.v}.
\end{case}
...
\textsf{FV}(\Gamma) \cap \{p,r,n\}  =\emptyset 
  \label{ahu3flk} \\
\rho.f  = \rho_1 \text{ and } \rho_1.f_4  = \rho_2 \text{ and } \N(f) = r
    \label{ahu4flk} \\
...
Proofs for \textbf{OW}, \textbf{RWOW}, \textbf{AWRT}, \textbf{IFL}, \textbf{WULK}, \textbf{FLR}, \textbf{FPI}, \textbf{WF}, \textbf{FR}, \textbf{HD}, \textbf{WNR}, \textbf{RINFL} and \textbf{ULKR} follow the corresponding cases of Lemma \ref{lemma:linkf} and Lemma \ref{lemma:unlink}.

\begin{case} - \textbf{UNQR}
Let $o_p$, $o_r$ and $o_n$ be $\sigma.s(p,tid)$, $\sigma.s(r,tid)$ and
$\sigma.s(n,tid)$.  This case is \emph{not} simpler than the corresponding cases
of Lemma \ref{lemma:linkf} and Lemma \ref{lemma:unlink}, and it does not follow
from the framing conditions and \textbf{FR} alone.  Insertion is the one mutation
that \emph{lengthens} paths: what was reached by $\rho.f.\tau$ before the write is
reached by $\rho.f.f_4.\tau$ after it, so an unchanged path and a lengthened one
are different objects, and the obligation is to rule out a collision between them.
The argument is mechanized as \texttt{UNQR\_insert} (and
\texttt{UNQR\_insert\_reachable}, which discharges its side condition from
reachability) in \texttt{rocq/HeapPaths.v}.  Its hypotheses are discharged as
follows.

\begin{enumerate}
\item \textbf{UNQR} holds before the write --- \ref{ahu16flk}.\textbf{UNQR}.
\item $o_p$ is reachable at $\rho$ and $\sigma.h(o_p,f) = o_r$ --- \ref{ahu4flk}
  ($\rho.f = \rho_1$ and $\N(f) = r$) with the denotation of
  $p:\textsf{rcuItr}\,\rho\,\N$ in \ref{ahu11flk}.
\item $o_n$'s only outgoing \textsf{RCU} edge is $f_4$, and it points at $o_r$.
  The rule gives $\N(f) = \N_1(f_4)$ and, by \ref{ahu5flk}, that every
  \emph{tracked} field of $\N_1$ other than $f_4$ is \textsf{null}.  On its own
  that leaves a field outside $dom(\N_1)$ free to point anywhere, which would give
  the inserted node a second child; it is closed by the clause of
  $\llbracket \textsf{rcuFresh}\,\N \rrbracket$ requiring every \textsf{RCU} field
  outside $dom(\N)$ to be \textsf{null}, established by Lemma \ref{lemma:alloc}
  and preserved by Lemma \ref{lemma:writef}.
\item $o_n$ is unreachable from the root, and in particular $o_n \neq o_p$ ---
  \ref{ahu16flk}.\textbf{FR}, via \texttt{no\_incoming\_unreachable} as in
  Lemma \ref{lemma:linkf}.
\item $o_p$ is not reachable from $o_r$ --- the tree shape, as in
  Lemma \ref{lemma:linkf}, mechanized as \texttt{UNQR\_no\_back\_edge}.
\end{enumerate}

Root is unchanged, $\sigma.rt = \sigma'.rt$, by operational semantics.
\end{case}
...
\end{case}
\begin{case} - \textbf{HD}
\textsc{ReadHeap} writes no field: the operational semantics change only the
stack, so $\sigma.h = \sigma'.h$.  \textbf{HD} is a property of the heap alone
and transfers unchanged from \ref{ahu16hr}.\textbf{HD}.
\end{case}
...
The clause of $\llbracket \textsf{rcuFresh}\,\N \rrbracket$ requiring every
\textsf{RCU} field outside $dom(\N)$ to be \textsf{null} is preserved.  The action
writes $p.f$, and the rule requires $f \notin dom(\N')$, so the post-state map is
$\N'([f\rightharpoonup z])$: the only field whose value changed is also the only
field added to the domain, and every other \textsf{RCU} field is untouched and
still outside the domain.  Since \textsc{Alloc} establishes the clause and this is
the only action that writes a field of a \textsf{rcuFresh} object, it holds
wherever a \textsf{rcuFresh} type is available --- which is what \textsc{T-Insert}
appeals to.
...
\end{case} 
\begin{case} - \textbf{HD}
The action writes $o_z$ into $o_p.f$, so the only new edge is
$\sigma'.h(o_p,f) = o_z$ and \textbf{HD} reduces to $o_z \in dom(\sigma'.h)$.
The rule requires $z:\textsf{rcuItr}\,\rho.f\,\_$, and the denotation of
$\textsf{rcuItr}$ places $o_z$ in the heap --- indeed reachable from the root,
which is stronger than needed here.  All other edges are inherited, where
\ref{ahu16wf}.\textbf{HD} applies.  Mechanized as \texttt{HD\_h\_upd}.
\end{case}
...
We assume $\sigma_1.h \bullet \sigma_2.h$. We know from \ref{ahu4wf} that $f \notin dom(\N')$.  The appeal to \textbf{RITR} here changes shape under the corrected statement.  The published version reads $\forall_{t,o}\ldotp t \in R \implies \textsf{iterator}\,t \in O(o)$ --- every reader observes \emph{every} location as \textsf{iterator} --- from which one can conclude anything, and which is outright contradictory in any state with a reader and a fresh node (\texttt{RITR\_orig\_contradicts\_fresh}); it cannot be what the step needs.  The corrected form instead says what a reader may \emph{not} observe: for $t \in R$, no location is observed by $t$ as \textsf{unlinked}, \textsf{freeable} or \textsf{fresh}.  That is the form used here, and it is enough: the location whose field is written is observed \textsf{fresh} by the writer, so by the corrected \textbf{RITR} no reader observes it at all, and by \ref{phu10wf}-\ref{phu11wf}.\textbf{WNR} the writer is not itself a reader.  The write therefore falls entirely outside the readers' fragment and cannot disturb the composition.  With that, and \ref{phu5wf} and \ref{phu10wf}-\ref{phu11wf}.\textbf{FNR}, we show $\sigma'_1.h \bullet \sigma'_2.h$ (with choices for other maps in the machine state we show \ref{ahusighr'}). All compositions shown  let us to derive conclusion for $(\sigma'_1, O'_1, U'_1, T'_1,F'_1) \bullet (\sigma'_2, O'_2, U'_2, T'_2,F'_2) $.
 \end{proof}
...
The field-map clauses of that denotation hold vacuously, since $dom(\N_{\emptyset})$
is empty.  Its remaining clause --- that every \textsf{RCU} field outside
$dom(\N)$ is \textsf{null} --- is exactly $\sigma' = \sigma[h(\ell) \mapsto
\textsf{nullmap}]$: the allocated object's fields are all \textsf{null}, and none
of them is tracked.  This is the base case of that clause; \textsc{WriteFreshField}
carries it forward.
...
\end{case} 
\begin{case} - \textbf{HD}
Allocation only enlarges the domain.  By \ref{phu5alc} the new location $\ell$
is mapped to \textsf{nullmap}, so it introduces no edge of its own and cannot
violate \textbf{HD}; and since $dom(\sigma'.h) = dom(\sigma.h) \cup \{\ell\}$,
every edge inherited from $\sigma$ still lands in the domain by
\ref{ahu16alc}.\textbf{HD}.  Note that freshness of $\ell$ is not what is doing
the work here --- it is needed for \textbf{FR} and \textbf{UNQR}, but
\textbf{HD} would survive even a re-initialisation.  Mechanized as
\texttt{HD\_h\_alloc}.
\end{case}
...
In the rest of the proof, we prove \ref{phu10free}, \ref{phu11free} and show the composition of $(\sigma'_1, O'_1, U'_1,T'_1,F'_1)$ and  $(\sigma'_2, O'_2, U'_2,T'_2,F'_2)$.. To prove \ref{phu10free}, we need to show that each of the memory axioms in Section \ref{sec:memaxioms} holds for the state $(\sigma',O'_1,U_1',T_1',F'_1)$.  All but one are immediate from \ref{ahusfree}-\ref{ahus4free} and \ref{phu5free}, since the action removes a location and adds nothing: an invariant whose quantifiers range over edges or over observations can only lose witnesses.

\textbf{HD} is the exception and needs its own argument, because \texttt{Free} is the only action that \emph{shrinks} $dom(\sigma.h)$, and \textbf{HD} constrains the target of every surviving edge.  Writing $o_d$ for the freed location, we need that no edge of $\sigma'.h$ points at $o_d$.  This does not follow from the operational semantics --- a state with a surviving pointer to the freed node satisfies \textbf{HD} before the free and violates it after, so the obligation is real (the counterexample is \texttt{HD\_h\_free\_needs\_no\_incoming} in \texttt{rocq/HeapPaths.v}).  It follows instead from the reachability form of \textbf{ULKR}: $x$ is typed \textsf{freeable}, so by \ref{ahu16free}.\textbf{ULKR} every predecessor of $o_d$ is itself \textsf{unlinked} or \textsf{freeable}, and by \ref{ahu16free}.\textbf{FLR} every such predecessor is on the free list.  By \ref{ahu16free}.\textbf{IFL} the free list is disjoint from the reachable structure, so no surviving edge of the data structure targets $o_d$.  Given that, \textbf{HD} is preserved; this is \texttt{HD\_h\_free}.
...
We know 
 ...
 We prove \ref{sl2p1} by from the consequence rule, based on the proofs of the following \ref{sl2p2} and \ref{sl2p3}
   ...
   The poof of \ref{sl2p2} follows from Views Logic's proof rule for assume construct by using
      ...
      as antecedent. We can use this antecedent together with the antecedent we know from \ref{sl2a3}
      \[\{\llbracket \Gamma \rrbracket_{M,tid} \}C\{\llbracket \Gamma \rrbracket_{M,tid}\}\]
