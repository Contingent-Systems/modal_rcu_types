...
if [ "${1:-}" = "--status" ]; then
  ...
  echo
  echo "  todo   Clang AST -> Stmt IR     the translation, and the only part"
  echo "                                  that needs this toolchain"
  echo "  todo   read __rcu on fields     to populate Config::rcuFields"
  echo "  todo   CFG with dominators      to identify back edges, which is what"
  echo "                                  closeBackEdge in rcu_check.h waits on"
  echo "  todo   diagnostic emission      the Result::why strings are already"
  echo "                                  written to be shown to a programmer"
  echo
